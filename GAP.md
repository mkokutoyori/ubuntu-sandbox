...
### 1.2 Duplication ARP/NDP — `NeighborResolver` écrit puis jamais branché — ✅ CORRIGÉ
...
- **Correction appliquée** : option « suppression » de la recommandation appliquée — `src/network/core/NeighborResolver.ts` (227 lignes) et son seul consommateur `src/__tests__/unit/events/NeighborResolver.scheduler.test.ts` (67 lignes) sont supprimés. Les ré-exports correspondants dans `src/network/core/index.ts:39-40` (`export { NeighborResolver }`, `export type { NeighborEntry }`) sont également retirés. Confirmé préalablement par `grep -rn 'NeighborResolver'` qu'aucun autre code ne consommait cette infrastructure (les seules occurrences restantes — `IpNeighborEntry` dans `linux-ip-command.test.ts`, `LldpNeighborEntry` dans `LldpAgent.ts` — sont des types nominalement homonymes mais entièrement disjoints). La factorisation prévue par cette classe « fantôme » sera mieux réalisée lors d'un futur refactor coordonné de `Router.queueAndResolve` ↔ `EndHost.fwdQueueAndResolve` (qui partagent désormais l'usage de `TimerSet` après la correction §1.7). 294 lignes de code mort retirées, `npx tsc --noEmit` propre.
