...
- **Correction appliquée** : `SQLPlusSession.processLine` route désormais `EXEC`/`EXECUTE` vers `executeSql` (qui descend dans `routePlsql` → `executeProcedureCall`/`callStoredUnit`), avec effets de bord réels (`DBMS_OUTPUT`, DML, erreurs). Au passage, l'exercice du chemin réel a révélé **deux bugs latents supplémentaires masqués par l'interception canned**, corrigés dans la même passe :
  ...
  - Validé par la suite complète `unit/database/` (2515 tests, 65 fichiers) + `debug/oracle/` (14 suites) — 0 régression.
