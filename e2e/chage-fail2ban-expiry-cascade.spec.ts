...
async function buildLab(page: Page): Promise<Lab> {
  return page.evaluate(() => {
    ...
    return Promise.resolve()
      ...
      .then(() => serverExec.call(serverDev, 'chage -M 1 -d 2000-01-01 alice'))
...
test.describe('Scénario 1 — chage expiration + cascade Fail2ban (UI réelle)', () => {
  test('un compte au mot de passe expiré déclenche des refus PAM puis un ban Fail2ban', async ({ page }) => {
    ...
    await waitForText(page, 'pam_unix(sshd:account): expired password for user alice');

    await typeCmd(page, 'fail2ban-client status sshd');
    await waitForText(page, 'Banned IP list:   10.0.0.1');

    await typeCmd(page, 'iptables -L f2b-sshd -n');
    await waitForText(page, 'REJECT');
  });
});
