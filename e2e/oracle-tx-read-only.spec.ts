/**
 * Oracle READ ONLY transactions & SET TRANSACTION — E2E (Playwright).
 *
 * `SET TRANSACTION READ ONLY` opens a read-only transaction that sees a
 * single consistent snapshot and rejects any DML with ORA-01456. It must be
 * the first statement of the transaction (else ORA-01453). `SET TRANSACTION
 * READ WRITE` and `... ISOLATION LEVEL SERIALIZABLE | READ COMMITTED` set the
 * mode for the next transaction.
 *
 * Assertions reflect real Oracle; divergences surface as failures.
 */
...
test.describe('Oracle READ ONLY / SET TRANSACTION', () => {
  ...
  test('DML inside a READ ONLY transaction raises ORA-01456', async ({ page }) => {
    ...
    // SELECT is allowed and sees the committed snapshot.
    expect(await sql(page, 'SELECT balance FROM t WHERE id = 1;')).toContain('100');

    // Any DML must be rejected.
  ...
  test('COMMIT ends the READ ONLY transaction and re-enables DML', async ({ page }) => {
    ...
    // End the read-only transaction; the next one is read-write again.
    expect(await sql(page, 'COMMIT;')).toContain('Commit complete.');
  ...
  test('SET TRANSACTION is rejected (ORA-01453) when it is not the first statement', async ({ page }) => {
    // Start a transaction with a DML statement…
    expect(await sql(page, 'UPDATE t SET balance = 300 WHERE id = 1;')).toContain('1 row updated.');
    // …then SET TRANSACTION can no longer apply to it.
