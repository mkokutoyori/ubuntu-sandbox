...
async function buildLab(page: Page): Promise<Lab> {
  return page.evaluate(() => {
    ...
    return Promise.resolve()
      ...
      .then(() => serverExec.call(serverDev, 'chage -M 30 -I 7 alice'))
      .then(() => serverExec.call(serverDev, `chage -E ${ymd} alice`))
...
test.describe('Scénario 2 — compte inactif (chage -I) et ban Fail2ban orthogonal (UI réelle)', () => {
  test('un compte déjà expiré est quand même à l\'origine d\'un ban Fail2ban de son IP source', async ({ page }) => {
    ...
    await page.evaluate(async ({ clientId }) => {
      ...
      for (let i = 0; i < 5; i++) {
        await exec.call(dev, 'sshpass -p wonderland ssh alice@10.0.0.2 whoami');
      }
    }, { clientId: client });

    await openTerminal(page, server);
    await typeCmd(page, 'cat /var/log/auth.log');
    await waitForText(page, 'pam_unix(sshd:account): account alice has expired (account expired)');

    await typeCmd(page, 'fail2ban-client status sshd');
    await waitForText(page, 'Banned IP list:   10.0.0.1');
  });
});
