...
test.describe('nice abaisse la commande, pas le shell', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/', { timeout: 60_000 });
    await waitForStore(page);
  });

  test('le shell garde sa priorite apres une commande niceee', async ({ page }) => {
    ...
    await typeCmd(page, 'nice -n 5 sleep 0');
    await typeCmd(page, 'nice');
    expect(await modalText(page)).toMatch(/\$ nice\s*\n\s*0\s*$/m);

    await typeCmd(page, 'clear');
    await typeCmd(page, 'ps -eo pid,ni,comm');
    const ligne = (await modalText(page)).split('\n').find((l) => l.includes('-bash')) ?? '';
    expect(ligne.trim().split(/\s+/)[1]).toBe('0');

    await closeTerminal(page);
  });

  test('la commande lancee, elle, porte bien l ajustement', async ({ page }) => {
    const pc = await addDevice(page, 'linux-pc', 340, 260);
    await openTerminal(page, pc);

    await typeCmd(page, 'clear');
    await typeCmd(page, 'nice -n 3 nice -n 4 nice');
    expect(await modalText(page)).toContain('7');

    await typeCmd(page, 'clear');
    await typeCmd(page, 'nice -n 5');
    expect(await modalText(page)).toContain('nice: a command must be given with an adjustment');
