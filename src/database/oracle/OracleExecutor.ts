...
export class OracleExecutor extends BaseExecutor {
  ...
  private evaluateFunction(expr: FunctionCallExpr, row: StorageRow, columns: StorageColMeta[]): CellValue {
    const name = expr.name.toUpperCase();
    const packageName = expr.schema?.toUpperCase() ?? null;
    const args = expr.args.map(a => this.evaluateExpression(a, row, columns));

    if (!packageName) {
      if (name === 'COUNT') {
        return args.length === 0 || (expr.args[0] && expr.args[0].type === 'Star')
          ? 1
          : (args[0] != null ? 1 : 0);
      }
    ...
    const impl = this.sqlFunctions.resolve(name, packageName);
    if (!impl) {
      const fullName = packageName ? `${packageName}.${name}` : name;
      throw new OracleError(904, `"${fullName}": invalid identifier`);
    }
    return impl(args, this.createFunctionContext());
  }

  private createFunctionContext(): SqlFunctionContext {
    return {
      currentUser: this.context.currentUser,
      currentSchema: this.context.currentSchema,
      ...
      userenv: (parameter) => {
        ...
        const value = session.userenv(parameter);
        return value === undefined ? undefined : (value as CellValue);
      },
