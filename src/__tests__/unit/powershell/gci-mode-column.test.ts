 ...
 * Directories should render as `d-----`. Migrated to use the interpreter
 * via PowerShellSubShell.
 */
...
describe('Get-ChildItem — Mode column', () => {
  it('newly created file has archive bit set (-a----)', async () => {
    ...
    await run(sh, 'New-Item -Path C:\\modeTest -ItemType Directory -Force');
    await run(sh, 'New-Item -Path C:\\modeTest\\hello.txt -ItemType File -Value "hi"');
    const out = await run(sh, 'Get-ChildItem C:\\modeTest');
    expect(out).toMatch(/-a----\s+.*hello\.txt/);
  });

  it('file created via Set-Content has archive bit set', async () => {
    const sh = createShell();
    await run(sh, 'New-Item -Path C:\\modeTest2 -ItemType Directory -Force');
    await run(sh, 'Set-Content -Path C:\\modeTest2\\note.txt -Value "x"');
    const out = await run(sh, 'Get-ChildItem C:\\modeTest2');
    expect(out).toMatch(/-a----\s+.*note\.txt/);
  });

  it('directory renders as d-----', async () => {
    const sh = createShell();
    await run(sh, 'New-Item -Path C:\\modeTest3 -ItemType Directory -Force');
    await run(sh, 'New-Item -Path C:\\modeTest3\\sub -ItemType Directory');
    const out = await run(sh, 'Get-ChildItem C:\\modeTest3');
    expect(out).toMatch(/d-----\s+.*sub/);
  });

  it('Mode column never has `------` for plain files (regression guard)', async () => {
    const sh = createShell();
    await run(sh, 'New-Item -Path C:\\modeTest4 -ItemType Directory -Force');
    await run(sh, '1..3 | ForEach-Object { New-Item -Path "C:\\modeTest4\\f$_.txt" -ItemType File -Value "row $_" }');
    const out = await run(sh, 'Get-ChildItem C:\\modeTest4');
