...
describe('Phase 6 — React hooks', () => {
  ...
  it('useArpTable re-renders when addStaticARP fires', () => {
    ...
    act(() => {
      pc.addStaticARP(new IPAddress('10.0.0.42'), new MACAddress('aa:bb:cc:dd:ee:ff'), 'eth0');
    });
    expect(result.current.length).toBe(1);
  ...
  it('useBusEvents accumulates events with a bounded ring buffer', () => {
    ...
    for (let i = 0; i < 5; i++) {
      act(() => {
        pc.addStaticARP(new IPAddress(`10.0.0.${i + 1}`), new MACAddress(`aa:bb:cc:dd:ee:0${i}`), 'eth0');
      });
    }
