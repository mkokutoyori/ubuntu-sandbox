...
describe('EndHost — ARP emissions on static add', () => {
  ...
  it('addStaticARP emits host.arp.entry-learned with source=static', () => {
    ctx.pc.addStaticARP(new IPAddress('10.0.0.42'), new MACAddress('aa:bb:cc:dd:ee:ff'), 'eth0');
  ...
  it('the stats signal counts the ARP cache size', () => {
    expect(ctx.pc.observables.stats.get().arpCacheSize).toBe(0);
    ctx.pc.addStaticARP(new IPAddress('10.0.0.42'), new MACAddress('aa:bb:cc:dd:ee:ff'), 'eth0');
    ctx.pc.addStaticARP(new IPAddress('10.0.0.43'), new MACAddress('aa:bb:cc:dd:ee:01'), 'eth0');
...
describe('HostCaptureActor — opt-in tcpdump-like recorder', () => {
  ...
  it('filters by deviceId', () => {
    ...
    ctx.pc.addStaticARP(new IPAddress('10.0.0.42'), new MACAddress('aa:bb:cc:dd:ee:ff'), 'eth0');
    ctx2.pc.addStaticARP(new IPAddress('10.0.0.43'), new MACAddress('aa:bb:cc:dd:ee:01'), 'eth0');
  ...
  it('caps the buffer at maxEntries', () => {
    ...
    small.start();
    for (let i = 0; i < 10; i++) {
      ctx.pc.addStaticARP(new IPAddress(`10.0.0.${i + 1}`), new MACAddress(`aa:bb:cc:dd:ee:${i.toString(16).padStart(2, '0')}`), 'eth0');
    }
