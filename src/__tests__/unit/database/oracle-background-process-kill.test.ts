...
describe('Killing a critical background process crashes the instance', () => {
  it('pkill -9 ora_pmon takes the instance down (ABORT)', async () => {
    const srv = boot('bg-1');
    const db = getOracleDatabase(srv.getId());
    expect(db.instance.state).toBe('OPEN');
    expect(await sh(srv, 'ps -ef')).toMatch(/ora_pmon/);

    await sh(srv, 'pkill -9 ora_pmon');

    expect(db.instance.state).toBe('SHUTDOWN');
    // Every background process is reaped from the host process table.
    expect(await sh(srv, 'ps -ef')).not.toMatch(/ora_pmon/);
    expect(await sh(srv, 'ps -ef')).not.toMatch(/ora_smon/);
  });

  it('killing SMON crashes the instance and records it in the alert log', async () => {
    const srv = boot('bg-2');
    const db = getOracleDatabase(srv.getId());

    await sh(srv, 'pkill -9 ora_smon');
  ...
  it('the crashed instance can be restarted afterwards', async () => {
    const srv = boot('bg-3');
    const db = getOracleDatabase(srv.getId());
    await sh(srv, 'pkill -9 ora_lgwr');
    expect(db.instance.state).toBe('SHUTDOWN');
...
describe('A non-terminating signal leaves the instance untouched', () => {
  it('pkill -CONT ora_pmon does not crash the instance', async () => {
    const srv = boot('bg-5');
    const db = getOracleDatabase(srv.getId());
    await sh(srv, 'pkill -CONT ora_pmon');
    expect(db.instance.state).toBe('OPEN');
  });
});
