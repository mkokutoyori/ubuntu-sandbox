...
describe('column additions', () => {
  it('DBA_TABLESPACES exposes DEF_TAB_COMPRESSION and RETENTION', async () => {
    const sh = s('dts');
    const out = (await run(sh, 'SELECT tablespace_name, def_tab_compression, retention FROM dba_tablespaces;'));
    expect(out).not.toMatch(/ORA-/);
    expect(out).toMatch(/DISABLED/);
    sh.dispose();
  });

  it('DBA_TABLES exposes CHAIN_CNT', async () => {
    const sh = s('chaincnt');
    const out = (await run(sh, "SELECT table_name, chain_cnt FROM dba_tables WHERE owner = 'HR';"));
    expect(out).not.toMatch(/ORA-/);
    sh.dispose();
  });
});
...
describe('DDL parser tolerance', () => {
  it('CREATE BIGFILE TEMPORARY TABLESPACE …', async () => {
    const sh = s('bigtemp');
  ...
  it('ALTER TABLE … ROW STORE COMPRESS / NOCOMPRESS', async () => {
    const sh = s('compress');
    (await run(sh, 'CREATE TABLE hr.t (id NUMBER);'));
    expect((await run(sh, 'ALTER TABLE hr.t ROW STORE COMPRESS ADVANCED;'))).toMatch(/Table altered/i);
    expect((await run(sh, 'ALTER TABLE hr.t NOCOMPRESS;'))).toMatch(/Table altered/i);
    sh.dispose();
  });

  it('ALTER INDEX … REBUILD ONLINE', async () => {
    const sh = s('reb');
    (await run(sh, 'CREATE TABLE hr.t (id NUMBER PRIMARY KEY);'));
    (await run(sh, 'CREATE INDEX hr.i ON hr.t (id);'));
    expect((await run(sh, 'ALTER INDEX hr.i REBUILD ONLINE;'))).toMatch(/Index altered/i);
    sh.dispose();
  });

  it('DROP TABLE … PURGE removes the table without recyclebin', async () => {
    const sh = s('purge');
    (await run(sh, 'CREATE TABLE hr.t (id NUMBER);'));
    expect((await run(sh, 'DROP TABLE hr.t PURGE;'))).toMatch(/Table dropped/i);
    sh.dispose();
  });
});
