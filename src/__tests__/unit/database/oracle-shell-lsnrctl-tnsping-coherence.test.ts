...
describe('shell lsnrctl/tnsping use the same real handler as the terminal', () => {
  ...
  it('lsnrctl status reflects the live listener state (database down → static UNKNOWN)', () => {
    ...
    const out = sh(srv, 'lsnrctl status');
    expect(out).toMatch(/Service "ORCL" has 1 instance/);
    expect(out).toMatch(/status UNKNOWN/);
    expect(out).not.toMatch(/status READY/);
  });

  it('lsnrctl status apres arret du listener lui-meme repond TNS-12541', () => {
    const srv = boot('shell-lsnr-3');
    sh(srv, 'lsnrctl stop');
    expect(sh(srv, 'lsnrctl status')).toMatch(/no listener|TNS-12541/i);
  });
