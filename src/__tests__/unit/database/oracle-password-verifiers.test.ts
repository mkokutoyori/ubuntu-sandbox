...
describe('SYS.USER$ password verifiers', () => {
  it('PASSWORD holds the real 10g DES hash (SCOTT/TIGER → F894844C34402B67)', async () => {
    const sh = newSession('ov-1');
    const out = (await run(sh, "SELECT PASSWORD FROM SYS.USER$ WHERE NAME='SCOTT';"));
    expect(out).toContain('F894844C34402B67');
    expect(out).not.toContain('TIGER'); // cleartext must not leak
    sh.dispose();
  });

  it('SPARE4 holds the 11g S: and 12c T: verifiers', async () => {
    const sh = newSession('ov-2');
    const out = (await run(sh, "SELECT SPARE4 FROM SYS.USER$ WHERE NAME='SCOTT';"));
  ...
  it('a freshly created user gets a real, deterministic 10g hash', async () => {
    const sh = newSession('ov-3');
    (await sh.processLine('CREATE USER VTEST IDENTIFIED BY tiger;'));
    const a = (await run(sh, "SELECT PASSWORD FROM SYS.USER$ WHERE NAME='VTEST';"));
    // Deterministic: SYS.USER$ renders the same hash on a second read.
    const b = (await run(sh, "SELECT PASSWORD FROM SYS.USER$ WHERE NAME='VTEST';"));
  ...
  it('USER_HISTORY$ stores a real 10g hash, not the cleartext', async () => {
    const sh = newSession('ov-4');
    (await sh.processLine('ALTER USER SCOTT IDENTIFIED BY tiger2;'));
    const out = (await run(sh, 'SELECT PASSWORD FROM SYS.USER_HISTORY$;'));
