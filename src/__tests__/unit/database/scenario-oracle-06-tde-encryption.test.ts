...
describe('creating an encrypted tablespace requires an open wallet with an active key', () => {
  it('fails with ORA-28365 when no wallet has ever been created', async () => {
  ...
  it('fails with ORA-28365 once the keystore exists but has no master key yet', async () => {
    const { subShell: s } = boot('tde2');
    (await sql(s, "ADMINISTER KEY MANAGEMENT CREATE KEYSTORE '/opt/oracle/wallet' IDENTIFIED BY \"WalletP@ss1\";"));
    const out = (await sql(s,
      "CREATE TABLESPACE secure_ts DATAFILE '/u01/app/oracle/oradata/ORCL/secure01.dbf' SIZE 1M ENCRYPTION USING 'AES256' ENCRYPT;"));
    expect(out).toMatch(/ORA-28365/);
    s.dispose();
  ...
  it('succeeds once the keystore is open and a master key is set', async () => {
    const { subShell: s } = boot('tde3');
    (await sql(s, "ADMINISTER KEY MANAGEMENT CREATE KEYSTORE '/opt/oracle/wallet' IDENTIFIED BY \"WalletP@ss1\";"));
    (await sql(s, "ADMINISTER KEY MANAGEMENT SET KEYSTORE OPEN IDENTIFIED BY \"WalletP@ss1\";"));
    (await sql(s, "ADMINISTER KEY MANAGEMENT SET KEY USING TAG 'master-2026' IDENTIFIED BY \"WalletP@ss1\" WITH BACKUP;"));

    const status = (await sql(s, 'SELECT status FROM v$encryption_wallet;'));
  ...
  it('closing the keystore blocks further encrypted tablespace creation again', async () => {
    const { subShell: s } = boot('tde4');
    (await sql(s, "ADMINISTER KEY MANAGEMENT CREATE KEYSTORE '/opt/oracle/wallet' IDENTIFIED BY \"WalletP@ss1\";"));
    (await sql(s, "ADMINISTER KEY MANAGEMENT SET KEYSTORE OPEN IDENTIFIED BY \"WalletP@ss1\";"));
    (await sql(s, "ADMINISTER KEY MANAGEMENT SET KEY USING TAG 'master-2026' IDENTIFIED BY \"WalletP@ss1\" WITH BACKUP;"));
    (await sql(s, "ADMINISTER KEY MANAGEMENT SET KEYSTORE CLOSE IDENTIFIED BY \"WalletP@ss1\";"));
...
describe('V$ENCRYPTED_TABLESPACES and DBA_TABLESPACES.ENCRYPTED reflect real state', () => {
  async function withOpenWallet(s: ReturnType<typeof boot>['subShell']) {
    (await sql(s, "ADMINISTER KEY MANAGEMENT CREATE KEYSTORE '/opt/oracle/wallet' IDENTIFIED BY \"WalletP@ss1\";"));
    (await sql(s, "ADMINISTER KEY MANAGEMENT SET KEYSTORE OPEN IDENTIFIED BY \"WalletP@ss1\";"));
    (await sql(s, "ADMINISTER KEY MANAGEMENT SET KEY USING TAG 'master-2026' IDENTIFIED BY \"WalletP@ss1\" WITH BACKUP;"));
  }

  it('an encrypted tablespace shows up in both views; a plain one does not', async () => {
    const { subShell: s } = boot('tde5');
    (await withOpenWallet(s));
    (await sql(s, "CREATE TABLESPACE secure_ts DATAFILE '/u01/app/oracle/oradata/ORCL/secure01.dbf' SIZE 1M ENCRYPTION USING 'AES256' ENCRYPT;"));
    (await sql(s, "CREATE TABLESPACE plain_ts DATAFILE '/u01/app/oracle/oradata/ORCL/plain01.dbf' SIZE 1M;"));

    const tbs = (await sql(s, "SELECT tablespace_name, encrypted FROM dba_tablespaces WHERE tablespace_name IN ('SECURE_TS','PLAIN_TS');"));
    expect(tbs).toMatch(/SECURE_TS\s+YES/);
    expect(tbs).toMatch(/PLAIN_TS\s+NO/);

    const enc = (await sql(s, 'SELECT encryptedts FROM v$encrypted_tablespaces;'));
    expect(enc).toContain('YES');
    expect(enc.match(/YES/g)?.length).toBe(1);
    s.dispose();
  });

  it('ALTER TABLESPACE ... ENCRYPTION ONLINE ENCRYPT re-encrypts an existing tablespace', async () => {
    const { subShell: s } = boot('tde6');
    (await withOpenWallet(s));
    (await sql(s, "CREATE TABLESPACE later_ts DATAFILE '/u01/app/oracle/oradata/ORCL/later01.dbf' SIZE 1M;"));
    expect((await sql(s, "SELECT encrypted FROM dba_tablespaces WHERE tablespace_name = 'LATER_TS';"))).toContain('NO');

    (await sql(s, 'ALTER TABLESPACE later_ts ENCRYPTION ONLINE ENCRYPT;'));
    expect((await sql(s, "SELECT encrypted FROM dba_tablespaces WHERE tablespace_name = 'LATER_TS';"))).toContain('YES');
    expect((await sql(s, 'SELECT COUNT(*) FROM v$encrypted_tablespaces;'))).not.toMatch(/^\s*0\s*$/m);
    s.dispose();
  });
});
...
describe('column-level ENCRYPT also requires an open wallet with an active key', () => {
  it('ALTER TABLE MODIFY (col ENCRYPT) fails before a key exists, succeeds after', async () => {
    const { subShell: s } = boot('tde9');
    (await sql(s, 'CREATE TABLE customers (id NUMBER, ssn VARCHAR2(11));'));

    const before = (await sql(s, 'ALTER TABLE customers MODIFY (ssn ENCRYPT);'));
    ...
    const after = (await sql(s, 'ALTER TABLE customers MODIFY (ssn ENCRYPT);'));
    expect(after).toContain('Table altered');

    const cols = (await sql(s, "SELECT table_name, column_name FROM dba_encrypted_columns WHERE table_name = 'CUSTOMERS';"));
...
describe('TDE is fully transparent to SQL: encrypted data reads and writes exactly like plaintext', () => {
  it('SELECT/INSERT against an encrypted tablespace behave identically to a normal one', async () => {
    const { subShell: s } = boot('tde10');
    (await sql(s, "ADMINISTER KEY MANAGEMENT CREATE KEYSTORE '/opt/oracle/wallet' IDENTIFIED BY \"WalletP@ss1\";"));
    (await sql(s, "ADMINISTER KEY MANAGEMENT SET KEYSTORE OPEN IDENTIFIED BY \"WalletP@ss1\";"));
    (await sql(s, "ADMINISTER KEY MANAGEMENT SET KEY USING TAG 'master-2026' IDENTIFIED BY \"WalletP@ss1\" WITH BACKUP;"));
    (await sql(s, "CREATE TABLESPACE secure_ts DATAFILE '/u01/app/oracle/oradata/ORCL/secure01.dbf' SIZE 1M ENCRYPTION USING 'AES256' ENCRYPT;"));
    const created = (await sql(s, 'CREATE TABLE vault (customer VARCHAR2(30)) TABLESPACE secure_ts;'));
    expect(created).toContain('Table created');
    const inserted = (await sql(s, "INSERT INTO vault VALUES ('BOB');"));
    expect(inserted).toMatch(/1 row created/);
    const rows = (await sql(s, 'SELECT customer FROM vault;'));
