...
describe('dot1x — system-auth-control and port modes', () => {
  it('a port left in auto mode starts unauthorized once system-auth-control is on', () => {
    ...
    expect(sw.getDot1xAgent().getPortRuntime('FastEthernet0/1')?.state).toBe('unauthorized');
  ...
  it('force-authorized always authorizes the port', () => {
    const sw = new CiscoSwitch('switch-cisco', 'SW', 4);
    sw.getDot1xAgent().setSystemAuthControl(true);
    sw.getDot1xAgent().setPortMode('FastEthernet0/1', 'force-authorized');
  ...
  it('force-unauthorized always blocks the port', () => {
    const sw = new CiscoSwitch('switch-cisco', 'SW', 4);
    sw.getDot1xAgent().setSystemAuthControl(true);
    sw.getDot1xAgent().setPortMode('FastEthernet0/1', 'force-unauthorized');
    expect(sw.getDot1xAgent().isPortAuthorized('FastEthernet0/1')).toBe(false);
  });
...
describe('dot1x — full handshake via local user DB', () => {
  it('EAPOL-Start → EAP-Request/Identity → EAP-Response/Identity → EAP-Success authorizes the port', () => {
    ...
    new Cable('c').connect(sw.getPort('FastEthernet0/1')!, supplicantSw.getPort('FastEthernet0/1')!);
    ...
    supplicantSw.getPort('FastEthernet0/1')!
...
describe('dot1x — EAPOL-Logoff', () => {
  it('moves an authorized port back to unauthorized and clears the identity', () => {
    ...
    supplicantSw.getPort('FastEthernet0/1')!.sendFrame(buildEapolLogoff(supplicantMac));
    const after = sw.getDot1xAgent().getPortRuntime('FastEthernet0/1')!;
...
describe('dot1x — held quiet-period timer (IEEE 802.1X §8.2)', () => {
  ...
  it('a held port returns to unauthorized after holdMs, re-authenticable', () => {
    ...
    new Cable('c').connect(sw.getPort('FastEthernet0/1')!, sup.getPort('FastEthernet0/1')!);
    const ag = sw.getDot1xAgent();
    ag.setSystemAuthControl(true);
    ag.setPortMode('FastEthernet0/1', 'auto');
    ...
    sup.getPort('FastEthernet0/1')!.sendFrame(buildEapolStart(mac));
    const rt = ag.getPortRuntime('FastEthernet0/1')!;
    sup.getPort('FastEthernet0/1')!
      .sendFrame(buildEapResponseIdentity(mac, rt.pendingEapId!, 'mallory'));
    sup.getPort('FastEthernet0/1')!
      .sendFrame(buildEapResponseIdentity(mac, rt.pendingEapId!, 'mallory'));
    expect(ag.getPortRuntime('FastEthernet0/1')?.state).toBe('held');
    ...
    expect(ag.getPortRuntime('FastEthernet0/1')?.state).toBe('unauthorized');
    expect(reasons).toContain('hold-expired:unauthorized');
    expect(ag.getPortRuntime('FastEthernet0/1')?.reauthCount).toBe(0);
  });
  ...
  it('a successful auth before the quiet period elapses cancels the held timer', () => {
    ...
    sup.getPort('FastEthernet0/1')!.sendFrame(buildEapolStart(mac));
    let rt = ag.getPortRuntime('FastEthernet0/1')!;
    sup.getPort('FastEthernet0/1')!
      .sendFrame(buildEapResponseIdentity(mac, rt.pendingEapId!, 'mallory'));
    sup.getPort('FastEthernet0/1')!
      .sendFrame(buildEapResponseIdentity(mac, rt.pendingEapId!, 'mallory'));
    ...
    sup.getPort('FastEthernet0/1')!.sendFrame(buildEapolStart(mac));
    rt = ag.getPortRuntime('FastEthernet0/1')!;
    sup.getPort('FastEthernet0/1')!
      .sendFrame(buildEapResponseIdentity(mac, rt.pendingEapId!, 'alice'));
    expect(ag.isPortAuthorized('FastEthernet0/1')).toBe(true);

    vi.advanceTimersByTime(120_000);
    expect(ag.getPortRuntime('FastEthernet0/1')?.state).toBe('authorized');
  });
});
...
describe('dot1x — de-authorization flushes the port MAC table', () => {
  it('an EAPOL-Logoff purges the dynamic MACs learned while authorized', () => {
    ...
    sup.getPort('FastEthernet0/1')!.sendFrame({
    ...
    sup.getPort('FastEthernet0/1')!.sendFrame(buildEapolLogoff(mac));

    expect(ag.isPortAuthorized('FastEthernet0/1')).toBe(false);
...
describe('dot1x — port enforcement against data frames', () => {
  it('an unauthorized auto port silently drops non-EAPOL frames at the switch ingress', () => {
    ...
    expect(sw.getDot1xAgent().isPortAuthorized('FastEthernet0/1')).toBe(false);
    expect(() => {
      supplicantSw.getPort('FastEthernet0/1')!.sendFrame({
        srcMAC: supplicantSw.getPort('FastEthernet0/1')!.getMAC(),
        dstMAC: MACAddress.broadcast(),
        etherType: 0x0800,
