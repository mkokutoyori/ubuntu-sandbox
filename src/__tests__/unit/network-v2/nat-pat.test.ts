...
describe('Group 11: ICMP embedded-packet re-translation (RFC 5508)', () => {
  ...
  it('11.1 inbound ICMP error re-translates embedded packet src (PAT)', () => {
    ...
    expect((innerResult!.payload as import('@/network/core/types').UDPPacket).sourcePort).toBe(5000);
    expect(result!.destinationIP.toString()).toBe('192.168.1.10');
  });

  it('11.2 an inside server\'s ICMP error leaves with its global address, outer and quoted alike', () => {
    engine.addStaticEntry({ localIP: '192.168.1.20', globalIP: '203.0.113.10' });

    const embeddedPkt: IPv4Packet = makeUDPPacket('1.2.3.4', '192.168.1.20', 9999, 80);
    const icmpError: ICMPPacket = {
      type: 'icmp', icmpType: 'destination-unreachable', code: 3,
      id: 0, sequence: 0, dataSize: 8,
      originalPacket: embeddedPkt,
    };
    const icmpPkt = createIPv4Packet(new IPAddress('192.168.1.20'), new IPAddress('1.2.3.4'), IP_PROTO_ICMP, 64, icmpError, 32);
    ...
    const innerResult = (result!.payload as ICMPPacket).originalPacket;
    expect(innerResult).toBeDefined();
    expect(innerResult!.destinationIP.toString()).toBe('203.0.113.10');
    expect(innerResult!.sourceIP.toString()).toBe('1.2.3.4');
  });
});
