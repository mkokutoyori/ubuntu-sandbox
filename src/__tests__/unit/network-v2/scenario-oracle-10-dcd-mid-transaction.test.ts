...
async function query(dbhost: LinuxServer, sql: string): Promise<string> {
  const a = admin(dbhost);
  const out = (await a.subShell.processLine(sql)).output.join('\n');
  a.subShell.dispose();
  return out;
}

/** Build a real DeadConnectionMonitor wired to the actual OracleDatabase + network reachability. */
async function makeMonitor(dbhost: LinuxServer) {
...
describe('Scénario 10 — Cohérence transactionnelle face à une coupure réseau mid-transaction', () => {
  it('sqlnet.ora expose bien SQLNET.EXPIRE_TIME côté serveur (DCD configuré)', async () => {
    const { dbhost } = lan();
    const monitor = (await makeMonitor(dbhost));
  ...
  it('une transaction distante ouverte (INSERT/UPDATE sans COMMIT) est invisible aux autres sessions avant coupure', async () => {
    ...
    (await query(dbhost, "CREATE TABLE system.orders (id NUMBER, status VARCHAR2(10));"));
    (await query(dbhost, "INSERT INTO system.orders VALUES (1, 'NEW');"));
    (await query(dbhost, 'COMMIT;'));

    const r = SqlPlusSubShell.create(client, ['system/oracle@ORCLDB']);
    (await r.subShell.processLine("INSERT INTO system.orders VALUES (2, 'NEW');"));
    (await r.subShell.processLine("UPDATE system.orders SET status = 'SHIPPED' WHERE id = 1;"));

    const seenByAdmin = (await query(dbhost, 'SELECT id, status FROM system.orders ORDER BY id;'));
  ...
  it("après coupure réseau, le sweep DCD détecte la session morte et l'élimine", async () => {
    ...
    const before = (await query(dbhost, "SELECT username FROM v$session WHERE username = 'SYSTEM';"));
    ...
    const after = (await query(dbhost, "SELECT username FROM v$session WHERE username = 'SYSTEM';"));
  ...
  it("le rollback automatique restaure les données à leur dernier état committé (aucune donnée partielle)", async () => {
    ...
    const rows = (await query(dbhost, 'SELECT id, status FROM system.orders ORDER BY id;'));
  ...
  it("v\\$transaction n'a plus d'entrée active pour la session morte après le sweep", async () => {
    ...
    const activeBefore = (await query(dbhost, 'SELECT status FROM v$transaction;'));
    ...
    const activeAfter = (await query(dbhost, 'SELECT status FROM v$transaction;'));
  ...
  it("aucun verrou résiduel dans v\\$lock après nettoyage de la session morte", async () => {
    ...
    const lockedBefore = (await query(dbhost, 'SELECT sid, type FROM v$lock;'));
    ...
    const lockedAfter = (await query(dbhost, 'SELECT sid, type FROM v$lock;'));
    expect(lockedAfter).toMatch(/no rows selected/i);

    const blockers = (await query(dbhost, 'SELECT * FROM dba_blockers;'));
  ...
  it("l'alert log documente le rollback automatique déclenché par la DCD", async () => {
    const { client, dbhost } = lan();
    const r = SqlPlusSubShell.create(client, ['system/oracle@ORCLDB']);
    (await r.subShell.processLine("CREATE TABLE probe (v NUMBER);"));
    (await r.subShell.processLine('INSERT INTO probe VALUES (1);'));
  ...
  it('une session locale (bequeath) sans transaction distante ne peut jamais être ciblée par la DCD', async () => {
    const { dbhost } = lan();
    (await query(dbhost, "CREATE TABLE probe2 (v NUMBER);"));
    const admin1 = SqlPlusSubShell.create(dbhost, ['/', 'as', 'sysdba']);
    (await admin1.subShell.processLine('INSERT INTO probe2 VALUES (1);'));

    const monitor = (await makeMonitor(dbhost));
    const killed = await monitor.check();
    expect(killed.length).toBe(0);

    const rows = (await admin1.subShell.processLine('SELECT v FROM probe2;')).output.join('\n');
  ...
  it('une session distante toujours joignable (réseau intact) survit au sweep DCD', async () => {
    ...
    const stillThere = (await query(dbhost, "SELECT username FROM v$session WHERE username = 'SYSTEM';"));
  ...
  it('la topologie publie oracle.session.dead-connection avec rolledBack=true pour la session tuée', async () => {
    ...
    const r = SqlPlusSubShell.create(client, ['system/oracle@ORCLDB']);
    (await r.subShell.processLine('CREATE TABLE probe3 (v NUMBER);'));
    (await r.subShell.processLine('INSERT INTO probe3 VALUES (1);'));
  ...
  it("désactiver l'interface DB host au lieu du client ne casse pas la session locale de l'admin", async () => {
    const { dbhost } = lan();
    (await query(dbhost, "CREATE TABLE probe4 (v NUMBER);"));
    const admin1 = SqlPlusSubShell.create(dbhost, ['/', 'as', 'sysdba']);
    (await admin1.subShell.processLine('INSERT INTO probe4 VALUES (7);'));
