...
function inviteUnMotDePasse(cmd: string): boolean {
  return /(^|[|;&]\s*)(\w+=\S*\s+)*(ssh|scp|sftp|stelnet)\s/.test(cmd) && !/\bsshpass\b/.test(cmd);
}
  ...
  const rows: Row[] = [
    {
      ...
      on: l => l.pc1,
      cmd: 'sshpass -p admin scp alice@10.0.0.2:/tmp/file.txt /tmp/local.txt',
    ...
    },
    {
      ...
      on: l => l.pc1,
      cmd: 'sshpass -p admin sftp alice@10.0.0.2',
    ...
    {
      name: 'scp vers une IP hors du reseau : "Network is unreachable"',
      on: l => l.pc1,
    ...
    },
    {
      ...
      on: l => l.pc1,
      cmd: 'sshpass -p admin scp -P 2222 /tmp/x alice@10.0.0.2:/tmp/',
