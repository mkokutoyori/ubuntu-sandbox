...
async function labo(): Promise<{ srv: LinuxPC; cli: LinuxPC; journal: () => Promise<string> }> {
  ...
  await cli.executeCommand('ssh alice@10.0.0.1 whoami', 'secret\n');
  await r.executeCommand('ssh -l alice 10.0.0.1', { passwordInput: 'secret' });
  await hw.executeCommand('stelnet 10.0.0.1', { passwordInput: 'secret' });
  await win.executeCommand('ssh alice@10.0.0.1', 'secret\n');
...
describe('toute session ouverte se referme', () => {
  ...
  it('mais une commande qui DURE tient sa session, et la rend a son terme', async () => {
    ...
    try {
      ...
      await cli.executeCommand('ssh alice@10.0.0.1 sleep 60', 'secret\n');
