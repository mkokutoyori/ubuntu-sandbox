...
export class SshConnectionRequest implements SshConnectionRequestSnapshot {
  ...
  static create(init: {
    ...
    return new SshConnectionRequest({
      ...
      requestedPort: init.requestedPort ?? 22,
      sourceIp: init.sourceIp,
      sourcePort: init.sourcePort ?? 0,
      sourceHostname: init.sourceHostname,
      clientVersion: init.clientVersion ?? 'SSH-2.0-OpenSSH_simulator',
      offeredAuthMethods: Object.freeze([...(init.offeredAuthMethods ?? ['publickey', 'password'])]),
      ...
      pty: init.pty ?? null,
      forwarding: init.forwarding ?? { agent: false, x11: false, locals: [], remotes: [], dynamics: [] },
