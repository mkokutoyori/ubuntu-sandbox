...
export interface ExecResult {
  ...
  /** The remote asked the client to wipe the screen (`cls`, `clear`). */
  readonly clearScreen?: boolean;
  /**
   * The remote's shell is waiting on one value — typically the password
   * for an `ssh` it started itself. The client collects it and answers
   * with {@link ISshShellChannel.provideInput}.
   */
  readonly pendingInput?: { kind: 'password' | 'text'; promptText: string };
  /**
   * The remote logged the session out on this line, using its own exit
   * word (`quit` at VRP user-view, `exit` on cmd). The client tears the
   * session down instead of matching exit words itself.
   */
...
export interface ISshShellChannel extends ISshChannel {
  ...
  /** Answer a challenge the remote raised through `ExecResult.pendingInput`. */
  ...
  /**
   * The remote's prompt as published when the channel opened, before a
   * single line ran — a vendor CLI shows `R1>` from the moment the
   * client lands (docs/PRD-SSH-Unification.md §4bis B4). Null when the
   * remote published none.
   */
  initialPrompt(): string | null;
  /**
   * True when the remote is a POSIX shell, which settles the behaviours
   * a client would otherwise assume: Ctrl+D is EOF, `clear` wipes the
   * screen. A cmd.exe remote ignores Ctrl+D and does not know `clear`;
   * on a vendor CLI `clear` is a real command.
   */
  isPosixShell(): boolean;
  /**
