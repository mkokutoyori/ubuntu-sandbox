  ...
  for (const [peerIp, sas] of ipsecSADB) {
    for (const sa of sas) {
      out.push({
        ...
        lifetimeSec: sa.lifetimeSeconds,
        lifetimeKB: sa.lifetimeKilobytes,
        bytesIn: sa.bytesIn ?? 0,
        bytesOut: sa.bytesOut ?? 0,
        packetsIn: sa.packetsIn ?? 0,
        packetsOut: sa.packetsOut ?? 0,
      });
    }
  }
  return out;
