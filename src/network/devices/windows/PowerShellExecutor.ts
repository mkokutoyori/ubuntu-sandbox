...
import { handleTestConnection as psHandleTestConnection, formatGetCimInstance as psFormatGetCimInstance, type PSDiagnosticContext } from './PSDiagnosticCmdlets';
import { handleNetshWinhttp as psHandleNetshWinhttp, handleNetshWlan as psHandleNetshWlan, type NetshLocalState } from './PSNetshCmdlets';
...
export class PowerShellExecutor {
  ...
  private netshState: NetshLocalState = { winhttpProxy: '', wlanConnectedSSID: '', wlanProfiles: new Set() };
  ...
  private async executeSingle(cmdline: string): Promise<string | null> {
    ...
    if (cmdLower === 'test-connection') {
      return psHandleTestConnection(this.buildPSDiagnosticCtx(), args);
    }
    ...
    if (cmdLower === 'netsh' && args[0]?.toLowerCase() === 'winhttp') {
      return psHandleNetshWinhttp(this.netshState, args.slice(1));
    }

    // Intercept netsh wlan before CMD delegation (PS-level WLAN state)
    if (cmdLower === 'netsh' && args[0]?.toLowerCase() === 'wlan') {
      return psHandleNetshWlan(this.netshState, args.slice(1));
    }
    ...
    if (cmdLower === 'get-wmiobject' || cmdLower === 'gwmi' || cmdLower === 'get-ciminstance') {
      return psFormatGetCimInstance(this.buildPSDiagnosticCtx(), args);
    }
  ...
  private buildPSDiagnosticCtx(): PSDiagnosticContext {
    return { device: this.device };
  }
