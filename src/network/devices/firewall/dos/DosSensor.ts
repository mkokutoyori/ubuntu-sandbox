...
import { familyCovers, type AnomalyKind, type AnomalySpec } from './AnomalyCatalog';
...
export class DosSensor {
  ...
  evaluate(
    ...
    if (!familyCovers(spec.family, subject.protocol)) return null;
  ...
  private countConcurrentSessions(spec: AnomalySpec, subject: DosSubject): number {
    ...
    const already = this.deps.sessions().find(session => {
      if (!familyCovers(spec.family, session.c2s.protocol)) return false;
