...
export class CiscoSwitchShell extends CiscoShellBase<CiscoSwitch> implements ISwitchShell {
  ...
  /**
   * `config-acl` ne sert plus qu'aux ACL ARP.
   *
   * Il servait AUSSI les listes IP, standard et etendue confondues : une
   * liste standard y proposait donc les protocoles et `evaluate`, qui
   * n'appartiennent qu'a une etendue. Les listes IP ont desormais leurs
  ...
  /**
   * `spanning-tree` GLOBALE est passee au socle : voir
   * `cisco/stpGlobalSpecs.ts`. Ce qui vivait ici etait un gestionnaire
   * glouton qui avalait ses onze mots-cles, plus un SECOND
   * enregistrement pour `spanning-tree mst` qui ne validait rien, plus
   * un troisieme pour `no spanning-tree` — trois corps pour une famille.
   */
  private stpGlobalHost(): StpGlobalHost {
    return {
  ...
  private aclHeadHost(): AclHeadHost {
    ...
    return {
      ...
      enterNamed: (kind, nom) => {
        ...
        this.mode = kind === 'standard' ? 'config-std-nacl' : 'config-ext-nacl';
        return '';
      },
