...
export function showSnmp(
  ...
  const lines: string[] = [
    ...
    `    ${s.requestedVariables} Number of requested variables`,
    `    ${s.alteredVariables} Number of altered variables`,
    `    ${s.getRequestPdus} Get-request PDUs`,
    `    ${s.getNextPdus} Get-next PDUs`,
    `    ${s.setRequestPdus} Set-request PDUs`,
    `${s.pktsOut} SNMP packets output`,
    `    ${s.tooBigs} Too big errors (Maximum packet size 1500)`,
    `    ${s.noSuchNames} No such name errors`,
    `    ${s.badValues} Bad values errors`,
    `    ${s.genErrs} General errors`,
