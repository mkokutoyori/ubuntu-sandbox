...
export const CISCO_IOS_PROMPTS: PromptMap = {
  ...
  'config-router-af':          '{host}(config-router-af)#',
  'config-vrf':                '{host}(config-vrf)#',
  'config-vlan':               '{host}(config-vlan)#',
  'config-track':              '{host}(config-track)#',
  'config-ipsla':              '{host}(config-ip-sla)#',
  'config-ipsla-echo':         '{host}(config-ip-sla-echo)#',
  'config-ipsla-icmpjitter':   '{host}(config-ip-sla-icmpjitter)#',
  'config-ipsla-jitter':       '{host}(config-ip-sla-jitter)#',
  'config-ipsla-udp':          '{host}(config-ip-sla-udp)#',
  'config-ipsla-tcp':          '{host}(config-ip-sla-tcp)#',
  'config-ipsla-http':         '{host}(config-ip-sla-http)#',
  'config-ipsla-dns':          '{host}(config-ip-sla-dns)#',
  'config-ipsla-pathecho':     '{host}(config-ip-sla-pathEcho)#',
  'config-ipsla-http-raw':     '{host}(config-ip-sla-http-raw)#',
  'config-route-map':          '{host}(config-route-map)#',
