 ...
 */
export class CiscoRouterGlobalCdpCommand extends BaseCommand {
  readonly descriptor: CommandDescriptor = {
    name: 'cdp',
    summary: 'Enable/configure CDP globally',
    usage: 'cdp {run}',
    args: [
      { name: 'sub', type: 'string', required: true, description: 'run' },
    ],
    options: [],
    privileges: OP,
    category: 'router',
  ...
  async execute(ctx: CommandContext): Promise<ExitCode> {
    ...
    const sub = ctx.args.get<string>('sub');
