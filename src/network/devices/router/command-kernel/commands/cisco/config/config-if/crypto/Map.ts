...
/**
 * `map` — TODO: une ligne d'intention métier.
 * Commande feuille standalone : `ctx.machine` est l'unique source de données, formatage inline.
 */
export class CiscoRouterConfigIfCryptoMapCommand extends BaseCommand {
  readonly descriptor: CommandDescriptor = {
    name: 'map',
    summary: 'Attach crypto map to interface',
    usage: 'crypto map <name>',
    args: [
      { name: 'name', type: 'string', required: true, description: 'Crypto map name' },
    ],
    options: [],
    privileges: OP,
    category: 'router',
