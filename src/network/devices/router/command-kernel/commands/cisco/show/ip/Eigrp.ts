...
/**
 * `eigrp` — TODO: une ligne d'intention métier.
 * Commande feuille standalone : `ctx.machine` est l'unique source de données, formatage inline.
 */
export class CiscoRouterShowIpEigrpCommand extends BaseCommand {
  readonly descriptor: CommandDescriptor = {
    name: 'eigrp',
    summary: 'Show EIGRP info',
    usage: 'show ip eigrp [neighbors|topology]',
    args: [
      { name: 'rest', type: 'string', required: false, variadic: true, description: 'EIGRP sub-view' },
    ],
    options: [],
    privileges: ANY,
    category: 'router',
