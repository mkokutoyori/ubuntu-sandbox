...
/**
 * `ping` — TODO: une ligne d'intention métier.
 * Commande feuille standalone : `ctx.machine` est l'unique source de données, formatage inline.
 */
export class CiscoRouterPingCommand extends BaseCommand {
  readonly descriptor: CommandDescriptor = {
    name: 'ping',
    summary: 'Send ICMP echo',
    usage: 'ping <target> [count <n>]',
    args: [
      { name: 'rest', type: 'string', required: true, variadic: true, description: 'target + options' },
    ],
    options: [],
    privileges: OP,
    category: 'router',
