...
/**
 * `rename` — TODO: une ligne d'intention métier.
 * Commande feuille standalone : `ctx.machine` est l'unique source de données, formatage inline.
 */
export class HuaweiRouterUserRenameCommand extends BaseCommand {
  readonly descriptor: CommandDescriptor = {
    name: 'rename',
    summary: 'Rename a file',
    usage: 'rename <old> <new>',
    args: [
      { name: 'rest', type: 'string', required: true, variadic: true, description: 'rename tokens' },
    ],
    options: [],
    privileges: OP,
    category: 'router',
