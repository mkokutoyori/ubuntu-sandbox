...
/**
 * `bgp` — transition de mode (push vers `bgp-view`).
 ...
 */
export class HuaweiRouterSysBgpCommand extends PushModeCommand {
  readonly descriptor: CommandDescriptor = {
    name: 'bgp',
    summary: 'Enter or create BGP configuration',
    usage: 'bgp <asn>',
    args: [
      { name: 'asn', type: 'int', required: true, description: 'AS number' },
    ],
    options: [],
    privileges: OP,
    category: 'cli-mode',
  ...
  readonly allowedModes = ['system-view'];
    ...
    const value = ctx.args.get<number>('asn');
    if (value === undefined || value === null) {
      await ctx.io.stderr.write("Error: Incomplete command found at '^' position.\n");
      return false;
    }
    (ctx.session as CliSession).promptFields.set('selectedBgpAsn', String(value));
    return true;
  }
