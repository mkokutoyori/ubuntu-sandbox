...
/**
 * `rip` — transition de mode (push vers `rip-view`).
 ...
 */
export class HuaweiRouterSysRipCommand extends PushModeCommand {
  readonly descriptor: CommandDescriptor = {
    name: 'rip',
    summary: 'Enter or create RIP configuration',
    usage: 'rip [<process-id>]',
    args: [
      { name: 'processId', type: 'int', required: false, description: 'process id' },
    ],
    options: [],
    privileges: OP,
    category: 'cli-mode',
  ...
  readonly allowedModes = ['system-view'];
    ...
    (ctx.session as CliSession).promptFields.set('selectedRipProcess', String(value));
    return true;
  }
