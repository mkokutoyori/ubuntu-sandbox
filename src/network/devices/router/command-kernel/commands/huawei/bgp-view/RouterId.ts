...
/**
 * `router-id` — TODO: une ligne d'intention métier.
 * Commande feuille standalone : `ctx.machine` est l'unique source de données, formatage inline.
 */
export class HuaweiRouterBgpRouterIdCommand extends BaseCommand {
  readonly descriptor: CommandDescriptor = {
    name: 'router-id',
    summary: 'Set BGP router-id',
    usage: 'router-id <A.B.C.D>',
    args: [
      { name: 'rid', type: 'string', required: true, description: 'router-id' },
    ],
    options: [],
    privileges: OP,
    category: 'router',
