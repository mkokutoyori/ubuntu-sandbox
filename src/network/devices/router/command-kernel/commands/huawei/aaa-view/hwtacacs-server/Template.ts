...
/**
 * `template` — transition de mode (push vers `hwtacacs-server-view`).
 ...
 */
export class HuaweiRouterAaaHwtacacsTemplatePushCommand extends PushModeCommand {
  readonly descriptor: CommandDescriptor = {
    name: 'template',
    summary: 'Enter an HWTACACS server template',
    usage: 'hwtacacs-server template <name>',
    args: [
      { name: 'rest', type: 'string', required: true, variadic: true, description: 'hwtacacs template tokens' },
    ],
    options: [],
    privileges: OP,
    category: 'cli-mode',
  ...
  readonly allowedModes = ['aaa-view'];
    ...
    (ctx.session as CliSession).promptFields.set('selectedHwtacacsTemplate', Array.isArray(value) ? value.join(' ') : String(value));
    return true;
  }

  protected targetMode(_ctx: CommandContext): string {
    return 'hwtacacs-server-view';
  }
}
