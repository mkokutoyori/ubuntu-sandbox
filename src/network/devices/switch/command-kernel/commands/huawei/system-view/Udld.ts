...
/**
 * `udld` — TODO: une ligne d'intention métier.
 * Commande feuille standalone : `ctx.machine` est l'unique source de données, formatage inline.
 */
export class HuaweiSwitchSysUdldCommand extends BaseCommand {
  readonly descriptor: CommandDescriptor = {
    name: 'udld',
    summary: 'UDLD global configuration',
    usage: 'udld {enable|aggressive}',
    args: [
      { name: 'rest', type: 'string', required: true, variadic: true, description: 'udld tokens' },
    ],
    options: [],
    privileges: OP,
    category: 'switch',
