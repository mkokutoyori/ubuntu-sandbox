...
/**
 * `copy` — TODO: une ligne d'intention métier.
 * Commande feuille standalone : `ctx.machine` est l'unique source de données, formatage inline.
 */
export class CiscoSwitchCopyCommand extends BaseCommand {
  readonly descriptor: CommandDescriptor = {
    name: 'copy',
    summary: 'Copy files between locations',
    usage: 'copy <src> <dst>',
    args: [
      { name: 'rest', type: 'string', required: true, variadic: true, description: 'src + dst' },
    ],
    options: [],
    privileges: OP,
    category: 'switch',
