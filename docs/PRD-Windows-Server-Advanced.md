# PRD — Windows Server avancé : tous les non-objectifs de PRD-Windows-Server.md sauf l'interface graphique et Hyper-V/virtualisation (AD DS/Kerberos/LDAP, AD CS, IIS avancé, DFS/DFSR, RDP, clustering, WSUS, impression réseau, activation)

**Version** : 2.0
**Date** : 2026-07-05
**Projet** : Ubuntu Sandbox — Module Windows Server avancé
**Auteur** : Claude Code
**Références normatives** : RFC 4120 (Kerberos V5), RFC 4121 (GSS-API Kerberos V5 — bind LDAP SASL/GSSAPI), RFC 4511/4512/4513 (LDAPv3 — **partiellement déjà livré**, cf. § 1.1), RFC 4514/4515 (DN, filtres — **déjà livrés**), MS-DRSR (réplication AD, spécification Microsoft propriétaire), MS-ADTS (forêt/partitions/schéma), MS-SFU (délégation Kerberos S4U2Proxy), MS-RDPBCGR (Remote Desktop Protocol — négociation de connexion, **pas** le pipeline graphique, cf. § 2.1.16), MS-WCCE/RFC 2986/5280 (Certificate Services — enrôlement, requêtes PKCS#10, certificats X.509, déjà couverts côté crypto par `src/network/pki/`), RFC 1179 (Line Printer Daemon — Print Services réseau), `docs/PRD-Windows-Server.md` (**déjà livré, P1-P11**), `docs/PRD-HTTP.md` (**déjà livré, P1-P13** — moteur HTTPS consommé par l'objectif IIS avancé), `docs/PRD-TLS.md`/`docs/PRD-RADIUS.md`/`docs/PRD-DNS.md` (**tous déjà livrés** — PKI, RADIUS, DNS consommés tels quels).

---

## Historique des révisions

- **v1.0** (2026-07-05) : périmètre initial — les trois premiers
  non-objectifs de `PRD-Windows-Server.md` §2.2 (réplication AD multi-DC,
  Kerberos complet, forêts/trusts/schéma/LDAP filaire complet).
- **v2.0** (2026-07-05) : **extension à la demande explicite de
  l'utilisateur** — *« ajoute tous les non-objectifs de ce PRD en
  objectif, à l'exception du non-objectif se rapportant à l'interface
  graphique »*. Ce document a temporairement couvert l'intégralité des
  non-objectifs de `PRD-Windows-Server.md` §2.2 à l'exception du seul GUI,
  y compris Hyper-V sous forme de plan de gestion réduit.
- **v2.1** (2026-07-05) : **retour en arrière explicite de l'utilisateur**
  — *« il faut exclure hyper-v et tout ce qui est hyperviseur »*. Hyper-V
  et toute forme de virtualisation/hyperviseur sont retirés des objectifs
  et replacés en non-objectif (§ 2.2), y compris le plan de gestion réduit
  envisagé en v2.0. Ce document couvre donc désormais l'intégralité des
  non-objectifs de `PRD-Windows-Server.md` §2.2, à l'exception de
  « Interface graphique Server Manager/MMC » **et** de Hyper-V, tous deux
  hors périmètre (§ 2.2). Un des sept non-objectifs d'origine
  (« freeradius/BIND côté Linux ») s'est avéré **déjà satisfait** par
  d'autres PRD frères déjà livrés (`PRD-RADIUS.md`, `PRD-DNS.md`) — voir
  § 1.1, aucun travail n'est requis à ce titre.

---

## 0. Contexte et portée du document

`docs/PRD-Windows-Server.md` §2.2 exclut explicitement **sept** chantiers de
son périmètre :

> - **Réplication AD multi-DC réelle** (USN, KCC, sites) — un second DC
>   partage l'annuaire par référence, sans protocole de réplication.
> - **Kerberos complet** (tickets TGT/TGS chiffrés, délégation) —
>   authentification domaine simplifiée ; `klist` n'affiche que des
>   tickets simulés.
> - **Forêts multiples, trusts, schéma extensible, LDAP filaire complet**
>   (recherche LDAP par port 389 limitée à ce que la jonction exige).
> - **Interface graphique Server Manager/MMC** — surface = CLI/PowerShell
>   uniquement, conformément au simulateur.
> - **RDP**, **clustering/failover**, **Hyper-V**, **WSUS**, **AD CS**
>   (certificats), **DFS/DFSR**, **IIS avancé** (app pools .NET, HTTPS/TLS,
>   modules), **imprimantes réseau complètes**, **activation/licences**.
> - **NPS avancé** (NAP, modèles de stratégie complexes, comptabilité SQL).
> - **freeradius/BIND côté Linux** — projets frères distincts.

**À la demande explicite de l'utilisateur, ce PRD promeut au rang
d'objectifs la quasi-totalité de ces non-objectifs, à l'exception de
l'interface graphique et de Hyper-V/toute virtualisation** (§2.2 — ce
dernier retiré des objectifs après un second arbitrage explicite de
l'utilisateur, voir « Historique des révisions »). Concrètement, ce
document couvre désormais, en plus des trois chantiers déjà analysés en
v1.0 (réplication multi-DC, Kerberos, forêts/trusts/schéma/LDAP filaire
complet — § 1 à § 2.1.1-12 ci-dessous, inchangés) :

- **AD CS** (Certificate Services) — § 2.1.13 ;
- **IIS avancé** (HTTPS/TLS, app pools, modules) — § 2.1.14 ;
- **DFS Namespaces + DFSR** — § 2.1.15 ;
- **RDP** (négociation de connexion, sans pipeline graphique) — § 2.1.16 ;
- **Clustering/failover (WSFC)** — § 2.1.17 ;
- **WSUS** — § 2.1.18 ;
- **Impression réseau (Print Services)** — § 2.1.19 ;
- **Activation/licences** — § 2.1.20 ;
- **NPS avancé** (modèles de stratégie complexes, comptabilité SQL — NAP
  explicitement exclu, § 2.2, motif documenté) — § 2.1.21 ;

et confirme qu'un huitième chantier — **freeradius/BIND côté Linux** — est
**déjà satisfait**, sans travail supplémentaire requis (§ 1.1). **Hyper-V et
toute forme d'hyperviseur restent explicitement hors périmètre** (§ 2.2),
y compris sous la forme d'un simple plan de gestion sans virtualisation
réelle.

Comme en v1.0, ce PRD ne rouvre aucune des phases déjà livrées de
`PRD-Windows-Server.md` (P1-P11) ni d'aucun PRD frère déjà livré
(`PRD-TLS.md`, `PRD-HTTP.md`, `PRD-RADIUS.md`, `PRD-DNS.md`) : il consomme
leurs moteurs tels qu'ils existent (`DirectoryStore`/`DirectoryTree`/
`LdapServer`/`LdapClient`, `src/network/pki/`, `HttpsServerSession`,
`RadiusServerAgent`, le moteur DNS) et les étend de façon additive.

Une clarification factuelle s'impose avant toute planification — l'état
réel du dépôt, vérifié fichier par fichier, diffère significativement de ce
que le texte des non-objectifs laisse penser, tantôt en creux, tantôt en
plein :

1. **« Un second DC partage l'annuaire par référence »** ne décrit **aucun
   code existant** — c'est une phrase d'intention jamais implémentée (aucune
   méthode `installADDSDomainController`, le test
   `windows-server-addsforest.test.ts` affirme même l'inverse). Réplication
   multi-DC est un chantier **entièrement greenfield**.
2. **« Authentification domaine simplifiée »** ne décrit pas un Kerberos ou
   un NTLM simplifié : l'authentification de domaine actuelle est un **bind
   LDAP simple, mot de passe en clair sur le fil**. Le port 88 n'est qu'une
   entrée de registre de ports. Kerberos est **entièrement greenfield**.
3. Le troisième point est un **mélange** : LDAP a un vrai codec fil BER/
   RFC 4511 (~1 460 lignes), mais il manque plusieurs opérations, et il
   n'existe ni schéma extensible, ni forêt multi-domaines, ni trust
   (greenfield pour ces quatre derniers).
4. **AD CS** est greenfield côté façade Windows (aucun cmdlet/service
   `certsrv`), mais **la cryptographie sous-jacente existe déjà et est
   mature** : `src/network/pki/` (`CertificateAuthority`, `X509Certificate`,
   `CertificateVerifier`, CRL, OCSP) est utilisé en production par
   `PRD-TLS.md`/`PRD-HTTP.md`(HTTPS)/`PRD-FTP-SFTP.md`(FTPS) — ce chantier
   n'ajoute donc **aucune primitive cryptographique nouvelle**, seulement
   une façade de rôle Windows par-dessus un moteur déjà éprouvé.
5. **IIS avancé** est également un mélange favorable : `WindowsIisRole.ts`
   a été migré par `PRD-HTTP.md` §5 P12 sur le vrai moteur RFC 9112
   (`Http1ServerSession`) — il ne manque que le raccordement à
   `HttpsServerSession` (déjà livré, déjà consommé par FTPS) pour le volet
   HTTPS/TLS. Les app pools/modules IIS, en revanche, sont entièrement
   absents.
6. **DFS** peut réutiliser `smbShares` (déjà livré, P3) comme cible de
   namespace, et le modèle USN/vecteur high-watermark de l'objectif 5
   (réplication AD) comme patron pour DFSR — pas une nouvelle conception
   de réplication de zéro.
7. **RDP, clustering/failover, WSUS, impression réseau, activation/
   licences, NPS avancé (au-delà de ce qui existe déjà)** sont tous
   **entièrement greenfield** — zéro code existant vérifié par recherche
   exhaustive dans le dépôt (§ 1.1).
8. **Hyper-V reste exclu** (§ 2.2) : ce simulateur n'a **aucune** notion
   d'imbrication hôte/invité dans son modèle de device (chaque `Equipment`
   est un pair simulé « bare-metal », pas un hôte pouvant contenir
   d'autres `EndHost`). Une virtualisation réelle (VM = instance `EndHost`
   imbriquée, commutateur virtuel = pont vers le réseau simulé existant)
   serait un changement architectural fondamental, potentiellement plus
   large que ce PRD dans son ensemble — hors périmètre à la fois pour une
   implémentation réelle et pour un simple plan de gestion sans
   virtualisation, à la demande explicite de l'utilisateur.
9. **freeradius/BIND côté Linux** s'avère, à la vérification, **déjà
   entièrement livré** : `src/network/devices/linux/bind9/` (7 fichiers,
   ~1 480 lignes — `named.conf` réel, zones, `rndc`) et
   `src/network/devices/linux/freeradius/` (`clients.conf`/`users` réels
   au-dessus du moteur `RadiusServerAgent`) sont déjà en production,
   livrés par `PRD-DNS.md`/`PRD-RADIUS.md`. **Aucun travail supplémentaire
   n'est requis pour ce point.**

Aucune ligne de code n'est écrite dans le cadre de ce document — il sert de
base à la planification et à la revue avant le premier commit TDD.

### 0.1 Chaîne de dépendances entre PRDs

```
PRD-Windows-Server.md — livré, P1 à P11         PRD-TLS.md — livré, P1-P11
   │  DirectoryStore/DirectoryTree/AdTypes,        │  src/network/pki/ (AD CS,
   │  LdapServer/LdapClient, WindowsIisRole         │  §2.1.13), TlsServerSession
   │  (déjà migré RFC 9112 par PRD-HTTP §5 P12)     │  (IIS HTTPS, §2.1.14)
   │                                                │
   │        PRD-HTTP.md — livré, P1-P13             │      PRD-RADIUS.md/PRD-DNS.md
   │        │  HttpsServerSession (IIS HTTPS)        │      │  — livrés ; freeradius/BIND
   │        │                                        │      │  Linux déjà satisfaits (§1.1)
   ▼        ▼                                        ▼      ▼
PRD-Windows-Server-Advanced.md                              ◄── VOUS ÊTES ICI
   │  consomme DirectoryTree/LdapServer/LdapClient (Kerberos, réplication,
   │  schéma, forêt, trust, LDAP filaire), src/network/pki/ (AD CS),
   │  HttpsServerSession (IIS avancé), smbShares (DFS Namespaces) —
   │  additivement, sans réécrire aucun de ces moteurs
   ▼
(aucun consommateur PRD identifié pour l'instant)
```

Ce PRD **n'a aucune dépendance bloquante** : tous ses prérequis
(`PRD-Windows-Server.md`, `PRD-TLS.md`, `PRD-HTTP.md`, `PRD-RADIUS.md`,
`PRD-DNS.md`) sont intégralement livrés. C'est un PRD terminal — aucun autre
PRD connu n'en dépend à ce jour. Comme `PRD-FTP-SFTP.md` avant lui, ce
document **ne s'inscrit pas** dans `docs/tls_quic_http_log.md` (scopé à la
seule triade TLS/QUIC/HTTP) — un journal de coordination dédié devra être
ouvert séparément si plusieurs agents doivent s'y coordonner.

---

## 1. Analyse de l'existant

### 1.1 Inventaire

| Fichier | Rôle actuel | Constat |
|---|---|---|
| `src/network/devices/windows/server/ad/DirectoryStore.ts` (498 l.) | Façade CRUD (`newUser`/`newGroup`/`newComputer`/`newOrgUnit`/`newGpo`) au-dessus de `DirectoryTree` | Un `DirectoryStore` = un domaine, un DC. Attributs AD-fidèles (`sAMAccountName`, `userAccountControl` avec vrais bits UAC, `groupType`, `member`/`memberOf` DN-valués). **Aucun champ USN, invocationId, ou horodatage d'écriture originelle** sur aucune entrée — les primitives minimales d'un modèle de réplication n'existent même pas en germe |
| `src/network/devices/windows/server/ad/AdTypes.ts` (69 l.) | Modèle d'objets (`AdUser`/`AdGroup`/`AdComputer`/`AdOrgUnit`) | Union fermée, pas de type `AttributeSchema`/`ObjectClassSchema`. Commentaire d'en-tête explicite : *« a deliberate 'LDAP-lite' subset: no schema extensibility... no multi-DC replication »* — confirmation directe des deux premiers non-objectifs. `DomainInfo.dcs: string[]` est un champ vestigial, jamais peuplé ni lu |
| `src/network/devices/WindowsServer.ts:136-151` (`installADDSForest`) | Promotion du **premier** DC d'un domaine | Refuse toute deuxième promotion (`this.directoryStore` déjà défini → erreur). Aucune méthode `installADDSDomainController` n'existe dans tout le dépôt |
| `src/__tests__/unit/network-v2/windows-server-addsforest.test.ts:49` | Test de promotion | `it('fails on a second promotion attempt', ...)` — confirme qu'aucun scénario multi-DC n'est even testé aujourd'hui |
| `src/network/devices/windows/WinDomainDiag.ts` (108 l.) | `nltest`/`dcdiag`/`klist` | `cmdKlist` (l. 88-107) formate des **chaînes figées** (`KerbTicket Encryption Type: AES-256-CTS-HMAC-SHA1-96`, `Start Time: (simulated)`) — zéro dérivation d'un ticket réel |
| `src/network/devices/windows/domain/DomainJoinClient.ts` / `DomainLogonClient.ts` | Jonction de domaine / logon interactif | Les deux dialoguent en **LDAP simple bind, mot de passe en clair sur le fil** (`dialLdap` → `TcpSocket` réel), contre `DirectoryStore.checkPassword()`/`checkComputerSecret()` (comparaison de chaîne, pas de hash NTLM, pas de dérivation de clé). Le commentaire de tête l'assume : *« the same real wire path a domain controller uses to validate any credential »* |
| `src/network/core/WellKnownPorts.ts:42`, `IanaServiceRegistry.ts:54` | Registre de ports | `{88: 'kerberos'}` — nom d'affichage seul, **aucun listener** sur ce port nulle part dans le dépôt |
| `src/network/devices/WindowsServer.ts:190,200-201` | SRV DNS `_kerberos._tcp.dc._msdcs`, service SCM `Kdc` | Purement cosmétique — un enregistrement DNS et un nom de service pour que `dcdiag`/`Get-Service Kdc` affichent « Running », sans aucun socket réel derrière |
| `src/network/devices/windows/server/ad/ldap/{Ber,LdapDN,LdapFilter,LdapMessage,DirectoryTree,LdapServer,LdapClient}.ts` (~1 460 l.) | Codec LDAP fil réel (RFC 4511) | BER générique (X.690, longueurs définies), DN (RFC 4514), filtres (RFC 4515) et enveloppe `LDAPMessage` réels. Opérations câblées : `bindRequest` (**simple uniquement**), `searchRequest`/`add`/`modify`/`del`/`compare`/`unbind`. **Absents** : `modifyDNRequest`, `extendedRequest` (donc pas de StartTLS LDAP), `abandonRequest`, le champ `controls` de l'enveloppe (donc pas de pagination RFC 2696, pas de contrôles étendus), les référés (`searchResultReference`) |
| `src/network/devices/windows/server/featureCatalog.ts:36-39` | Catalogue de fonctionnalités | `AD-Domain-Services` est un rôle unique (services `NTDS`/`Netlogon`/`Kdc`, module PS `ActiveDirectory`) — aucune fonctionnalité de niveau forêt, aucun cmdlet de trust dans `ActiveDirectoryCmdlets.ts` |
| `DirectoryTree.search()` | Moteur de requête | Générique sur l'arbre qu'on lui donne (pas de restriction artificielle de filtre) — la limite est **structurelle** : un seul arbre par DC, donc une seule racine, un seul domaine, aucune partition (`cn=schema`, `cn=configuration`) |
| `src/network/pki/{CertificateAuthority,X509Certificate,CertificateVerifier,PkiKeyPair,CertificateRevocationList,OcspResponder}.ts` | Moteur PKI générique | **Mature et en production** — consommé par `PRD-TLS.md`, HTTPS (`PRD-HTTP.md`), FTPS (`PRD-FTP-SFTP.md`). Aucune façade « rôle Windows AD CS » (pas de service `certsrv`, pas de cmdlet `Install-AdcsCertificationAuthority`/`Get-Certificate`, pas de modèle de certificat) |
| `src/network/devices/windows/server/iis/WindowsIisRole.ts` | Rôle IIS | Migré par `PRD-HTTP.md` §5 P12 sur `Http1ServerSession` (RFC 9112 réel). **Aucun binding HTTPS/TLS** (pas de port 443, pas de `HttpsServerSession`), aucun modèle d'app pool, aucun registre de module |
| `WindowsServer.ts` (`smbShares`) | Partage SMB (P3) | Solide, un partage = un chemin physique local. **Aucune notion de namespace DFS** (un chemin logique agrégeant plusieurs cibles), **aucune réplication de fichiers** entre serveurs |
| Recherche exhaustive (`grep -rli`) | RDP, clustering/WSFC, WSUS, impression réseau, activation/licences | **Zéro fichier** correspondant à l'un de ces cinq chantiers dans tout `src/network/` — confirmé par recherche insensible à la casse sur les noms de rôles/cmdlets/protocoles concernés |
| `src/network/devices/windows/WinPrint.ts` | Commande `print` cliente | Existe déjà côté **client local** (soumission de job, accusé de réception, file locale) — *pas* un rôle serveur d'impression réseau ; confirme précisément la portée du non-objectif (« imprimantes réseau **complètes** ») |
| `src/network/devices/linux/bind9/{Bind9Service,NamedConfParser,NamedConfig,NamedAcl,RndcChannel,NamedConfLexer}.ts` (~1 480 l.) | Serveur DNS Linux (BIND9) | **Réel et livré** : `named.conf` parsé, zones, ACL, `rndc` — référence explicite à `PRD-DNS.md`/`PRD-RADIUS.md` dans les en-têtes. Le non-objectif « BIND côté Linux » est **déjà satisfait** |
| `src/network/devices/linux/freeradius/{RadiusdService,ClientsConfParser,UsersFileParser}.ts` | Serveur RADIUS Linux (freeradius) | **Réel et livré** : `clients.conf`/`users` réels au-dessus de `RadiusServerAgent`. En-tête : *« PRD-RADIUS §2.2 "L'implémentation freeradius sur Linux" »* — non-objectif **déjà satisfait**, aucun travail requis |

### 1.2 Ce qui existe déjà et est réutilisable

- **`DirectoryTree`/`DirectoryStore`** — le DIT complet (add/delete/modify/
  search/compare, hiérarchie parent-enfant par DN) est solide et déjà
  utilisé par le codec LDAP fil ; ce PRD lui ajoute des champs (USN,
  invocationId) et une partition schéma **de façon additive**, sans
  réécrire le moteur d'arbre.
- **`ad/ldap/*` en entier** (`Ber.ts`, `LdapDN.ts`, `LdapFilter.ts`,
  `LdapMessage.ts`, `LdapServer.ts`, `LdapClient.ts`) — le codec BER/
  RFC 4511 réel est directement réutilisable pour : (a) transporter le
  bind SASL/GSSAPI Kerberos comme deuxième `AuthenticationChoice` à côté
  du bind simple existant (sans le retirer, § 2.2 de `PRD-Windows-Server.md`
  reste satisfait pour les usages qui n'ont pas besoin de Kerberos) ; (b)
  ajouter les opérations manquantes (`modifyDN`/`extended`/`abandon`/
  `controls`) comme de nouveaux `CHOICE` dans l'enveloppe déjà réelle,
  pas un nouveau protocole.
- **`DomainJoinClient`/`DomainLogonClient`** — la logique de haut niveau
  (résolution DNS du DC, construction de l'objet ordinateur/session) est
  conservée ; seul le mécanisme d'authentification qu'ils invoquent
  change (Kerberos AS/TGS au lieu du bind LDAP direct, § 5 P24).
- **`EventBus`/`Signal`** (`src/events/`) — convention d'observabilité déjà
  utilisée par TLS/QUIC/HTTP/RADIUS, réutilisée telle quelle pour
  `kerberos.*`/`replication.*`/`trust.*`.
- **Convention « crypto simulée, forme du protocole réelle »** déjà établie
  par `PkiKeyPair`, `SimulatedTls.ts`, le key schedule de `PRD-TLS.md`,
  la protection de paquets de `PRD-QUIC.md` — directement applicable au
  chiffrement des tickets Kerberos (encodage ASN.1 DER réel des structures
  `KDC-REQ`/`KDC-REP`/`Ticket`/`EncTicketPart`, contenu chiffré simulé).
- **`featureCatalog.ts`/`RoleManager.ts`** — le modèle de fonctionnalités
  existant accueille une nouvelle fonctionnalité (« forêt »/« site
  supplémentaire », « AD CS », « Print Services », etc.) sans changement de
  son moteur de dépendances/gating.
- **`src/network/pki/` en entier** — réutilisé strictement tel quel pour
  AD CS (§ 2.1.13) : `CertificateAuthority.issueCertificate()` devient
  l'implémentation du rôle CA Windows, aucune nouvelle primitive
  cryptographique.
- **`HttpsServerSession`/`TlsServerSession`** (déjà livrés) — réutilisés
  tels quels pour IIS avancé (§ 2.1.14), exactement comme FTPS (P8 de
  `PRD-FTP-SFTP.md`) les a déjà réutilisés pour un protocole différent.
- **`smbShares`** — cible de namespace DFS (§ 2.1.15) sans changement de
  son modèle de partage.
- **Le modèle USN/vecteur high-watermark de l'objectif 5** (réplication AD)
  — réutilisé comme patron pour DFSR (§ 2.1.15), pas une seconde
  conception de réplication indépendante.
- **`WinPrint.ts`** (soumission de job côté client) — le rôle serveur
  d'impression réseau (§ 2.1.19) consomme la même structure `PrintJob`
  plutôt que d'en inventer une nouvelle.

### 1.3 Ce qui est non conforme ou manquant (gap analysis)

| # | Manque | Référence | Sévérité |
|---|---|---|---|
| 1 | Aucune réplication inter-DC (USN, vecteur high-watermark, cycle de synchronisation) | MS-DRSR | Bloquant |
| 2 | Aucune méthode de promotion d'un **second** DC dans un domaine existant | MS-DRSR / dcpromo | Bloquant |
| 3 | Aucun KDC réel — pas d'AS-REQ/AS-REP, pas de TGS-REQ/TGS-REP, pas de ticket chiffré | RFC 4120 §3.1/§3.3 | Bloquant |
| 4 | `klist`/authentification domaine reposent sur des chaînes cosmétiques et un bind LDAP en clair, pas sur un ticket réel | RFC 4120 | Bloquant |
| 5 | Aucune délégation Kerberos (contrainte ou non) | RFC 4120 / MS-SFU | Moyenne |
| 6 | Le bind LDAP est toujours `simple` — pas de mécanisme SASL/GSSAPI, donc pas de bind adossé à un ticket Kerberos | RFC 4513 §5.2 | Élevée (bloque l'objectif 3) |
| 7 | Aucun schéma extensible — pas de partition `cn=schema`, pas d'`attributeSchema`/`classSchema`, classes d'objet figées dans le code de `DirectoryStore` | RFC 4512 | Élevée |
| 8 | Aucune forêt multi-domaines — un `DirectoryStore` = un domaine, sans partition de configuration ni relation parent-enfant/arborescence de domaines | MS-ADTS | Élevée |
| 9 | Aucun trust inter-domaine/inter-forêt, aucun référencement cross-realm Kerberos | RFC 4120 §3.3.3 / MS-LSAT | Moyenne |
| 10 | LDAP : pas de `modifyDNRequest` (renommage/déplacement), `extendedRequest` (StartTLS), `abandonRequest`, champ `controls` (donc pas de pagination ni de contrôles étendus), pas de référés | RFC 4511 §4.9/§4.12/§4.11/§4.1.11/§4.5.2/§4.1.10 | Moyenne à faible selon l'opération |
| 11 | Port 88 enregistré (nom d'affichage) mais **aucun listener** — même remarque déjà faite pour d'autres protocoles avant leur propre PRD (FTP, RADIUS) | — | Bloquant (prérequis structurel de l'objectif 1) |
| 12 | Aucune façade AD CS (service, cmdlets, modèles de certificat) au-dessus du moteur PKI déjà mature | MS-WCCE | Élevée |
| 13 | IIS n'a aucun binding HTTPS/TLS, aucun app pool, aucun registre de module | — | Élevée (HTTPS), faible (app pools/modules) |
| 14 | Aucun namespace DFS, aucune réplication de fichiers (DFSR) | MS-DFSNM/MS-FRS2 | Moyenne |
| 15 | Aucune négociation de connexion RDP (TPKT/X.224/CredSSP) | MS-RDPBCGR | Moyenne |
| 16 | Aucun modèle de cluster (adhésion, quorum, bascule de groupe de ressources) | MS-CMRP | Moyenne |
| 17 | Aucun catalogue/approbation de mises à jour (WSUS) | — | Faible |
| 18 | Aucun rôle d'impression réseau (file partagée, protocole LPD) | RFC 1179 | Faible |
| 19 | Aucun état d'activation/licence simulé | — | Faible |
| 20 | NPS : pas de modèles de stratégie de demande de connexion multi-conditions, pas de comptabilité SQL ; NAP absent (et **délibérément non comblé**, § 2.2 — fonctionnalité retirée de Windows Server depuis 2012 R2) | — | Faible |

**Conclusion de la phase d'analyse** : Kerberos, la réplication multi-DC,
RDP, le clustering, WSUS, l'impression réseau et l'activation/licences sont
des chantiers **entièrement greenfield**, comparables à l'état de FTP avant
`PRD-FTP-SFTP.md`. AD CS et IIS avancé (HTTPS) sont greenfield côté façade
Windows mais s'appuient sur des moteurs déjà mûrs et déjà réutilisés
ailleurs (`src/network/pki/`, `HttpsServerSession`) — le travail restant est
une façade, pas une nouvelle primitive. Le schéma extensible, la forêt
multi-domaines et DFS/DFSR sont également greenfield, mais s'appuient sur
des moteurs (`DirectoryTree`, le modèle USN de l'objectif 5) déjà solides.
Le complément LDAP filaire est un renforcement d'un protocole déjà réel.
Hyper-V reste hors périmètre (§ 2.2) — son absence n'est donc pas traitée
comme un manque de ce document. freeradius/BIND côté Linux est déjà livré.
Toutes ces familles de travail sont largement indépendantes entre elles au
niveau du code, mais ont des dépendances internes fortes dans l'ordre
logique à l'intérieur de chaque famille (détaillé § 5).

---

## 2. Objectifs

### 2.1 Objectifs

**1. RFC 4120 §3.1 — KDC minimal, échange AS (Authentication Service).**
Un vrai listener UDP/TCP 88 (comblant le gap #11) implémentant `AS-REQ`/
`AS-REP` : pré-authentification par horodatage chiffré
(`PA-ENC-TIMESTAMP`, §5.2.7.2), émission d'un TGT (`Ticket` chiffré pour le
principal `krbtgt/RÉALME`, créé automatiquement à la promotion du DC comme
une entrée `DirectoryTree` de plus — pas une structure parallèle).
Encodage ASN.1 DER réel des structures `KDC-REQ`/`KDC-REP`/`Ticket`/
`EncTicketPart` (§5.4/§5.3) ; le contenu chiffré est simulé (même
convention que `TlsRecordWire`/`packetProtection.ts`), la forme du message
est réelle.

**2. RFC 4120 §3.3 — Échange TGS (Ticket-Granting Service).** `TGS-REQ`/
`TGS-REP` : un client présentant un TGT valide obtient un ticket de service
pour un SPN (`host/`, `ldap/`, `cifs/`, …) sans re-solliciter le mot de
passe. `klist` (remplace le gap #4) affiche des champs **réellement dérivés**
du ticket en cache (heures de début/fin/renouvellement, drapeaux, type de
clé de session) au lieu de chaînes figées.

**3. RFC 4513 §5.2 — Bind LDAP SASL/GSSAPI.** Le bind `simple` existant
(RFC 4513 §5.1) reste ; ce PRD ajoute le mécanisme `GSSAPI` comme deuxième
`AuthenticationChoice` de l'enveloppe LDAP déjà réelle (comble le gap #6) :
un client muni d'un ticket de service `ldap/<dc>` peut se lier sans
transmettre de mot de passe sur le fil. `DomainJoinClient`/
`DomainLogonClient` basculent sur ce mécanisme (§ 5 P24) — l'authentification
de domaine devient réellement Kerberos, pas un bind en clair déguisé.

**4. Délégation Kerberos contrainte minimale (S4U2Proxy).** Un service
peut obtenir, pour le compte d'un utilisateur déjà authentifié auprès de
lui, un ticket vers un second service explicitement autorisé (attribut
`msDS-AllowedToDelegateTo`, déjà représentable comme attribut d'objet
ordinateur) — le scénario double-saut WinRM/SMB déjà documenté par
`PRD-Windows-Server.md`. `S4U2Self` (délégation non contrainte complète)
reste hors périmètre (§ 2.2).

**5. Réplication AD multi-DC — modèle USN/vecteur high-watermark.**
Chaque `DirectoryStore` reçoit un `invocationId` et une USN locale
incrémentée à chaque écriture ; chaque attribut modifié porte
l'USN/l'horodatage/l'identifiant du DC d'origine (`originating write`,
inspiré de MS-DRSR sans viser sa conformité fil-à-fil). Un cycle de
réplication (« pull » déclenché manuellement ou à intervalle fixe,
**pas** de KCC calculant automatiquement une topologie, § 2.2) échange les
vecteurs high-watermark entre deux DC et ne transfère que les objets/
attributs modifiés depuis la dernière synchronisation.

**6. `Install-ADDSDomainController` — promotion d'un second DC.** Comble
le gap #2 : un second `WindowsServer` rejoint un domaine **existant**
(pas un nouveau domaine), déclenche une synchronisation initiale complète
depuis un DC désigné, puis participe au cycle de réplication de
l'objectif 5. `Get-ADDomainController` liste tous les DC connus (remplace
le champ vestigial `DomainInfo.dcs`).

**7. Sites — métadonnées minimales.** `New-ADReplicationSite`/
`Get-ADReplicationSite` définissent des sites nommés, chacun associé à un
sous-réseau ; un DC appartient à un site. Utilisé uniquement pour annoter
les journaux de réplication (« intra-site » vs « inter-site ») — **aucun
calcul réel de coût de lien ni de planification horaire** (§ 2.2).

**8. RFC 4512 — Schéma extensible.** Une partition `cn=schema,cn=
configuration,<domaine>` matérialisée comme sous-arbre `DirectoryTree`
ordinaire, peuplée d'objets `attributeSchema`/`classSchema` réels.
`New-ADObject -Type attributeSchema`/`New-ADObject -Type classSchema` (ou
cmdlets dédiés `New-ADAttribute`/`New-ADObjectClass`) créent de nouveaux
types ; `DirectoryTree.addEntry`/`modifyEntry` valident désormais un objet
contre le schéma effectivement présent dans cette partition plutôt que
contre un ensemble figé de classes codées en dur dans `DirectoryStore`
(remplace le gap #7), sans casser les classes déjà livrées (`user`,
`group`, `computer`, `organizationalUnit`, …, qui deviennent les entrées
schéma par défaut plutôt que des cas spéciaux).

**9. Forêt multi-domaines.** Une partition de configuration partagée entre
plusieurs `DirectoryStore` (relation arborescente parent-enfant ou racine
d'arborescence — `New-ADDomain -NewDomainName ... -ParentDomainName ...`),
un niveau fonctionnel de forêt, et le partage effectif de la même
partition schéma (objectif 8) entre tous les domaines de la forêt —
c'est *parce que* le schéma est une partition répliquée comme une autre
(objectif 5) que « une forêt = un schéma partagé » se réalise sans
mécanisme supplémentaire.

**10. Trust inter-domaine/inter-forêt.** `netdom trust`/`New-ADTrust`
établit une relation de confiance simple (bidirectionnelle, transitive)
entre deux royaumes Kerberos ; un utilisateur du domaine A authentifié
auprès de son propre KDC obtient, via un référé cross-realm (RFC 4120
§3.3.3 — le KDC de A renvoie un TGT inter-domaine utilisable auprès du KDC
de B), l'accès à une ressource du domaine B sans ressaisir ses
identifiants. Comble le gap #9.

**11. LDAP filaire complet — opérations restantes.** Ajout, dans
l'enveloppe `LDAPMessage` déjà réelle : `modifyDNRequest`/
`modifyDNResponse` (renommage/déplacement d'objet, utile pour les
opérations AD comme `Move-ADObject`), `extendedRequest`/`extendedResponse`
(au minimum l'OID StartTLS, RFC 4511 §4.14.1, consommant le moteur TLS déjà
livré), `abandonRequest`, le champ `controls` de l'enveloppe avec au moins
le contrôle de pagination des résultats (RFC 2696), et les référés
(`searchResultReference`) — ces derniers directement utiles pour une
recherche traversant plusieurs domaines de la forêt (objectif 9). Comble
le gap #10.

**12. Observabilité (AD DS/Kerberos).** `src/network/kerberos/events.ts`/
`observables.ts` (`kerberos.as.*`/`kerberos.tgs.*`/`kerberos.delegation.*`)
et `src/network/devices/windows/server/ad/replication/events.ts`/
`observables.ts` (`replication.cycle.*`/`replication.conflict.*`), à
l'image de la convention déjà en place pour TLS/QUIC/HTTP/RADIUS.

**13. MS-WCCE — AD CS (Certificate Services).** Un rôle `AD-Certificate`
exposant le service `CertSvc`, les cmdlets
`Install-AdcsCertificationAuthority`/`Get-Certificate`/`Get-CATemplate`,
la commande `certreq -submit`/`certutil -submit`, et un magasin minimal de
modèles de certificat (nom, EKU, durée de validité). **Aucune nouvelle
primitive cryptographique** : le rôle instancie une
`CertificateAuthority` (`src/network/pki/`, déjà livrée) comme CA racine
d'entreprise et lui délègue entièrement l'émission/signature — la façade
Windows (service, cmdlets, gabarits) est la seule chose que ce PRD ajoute.
Intégration directe avec l'objectif 14 (IIS avancé) : un certificat émis
par cette CA peut être lié à un site IIS pour son binding HTTPS.

**14. IIS avancé.**
- **HTTPS/TLS** : port 443, `New-WebBinding -Protocol https -CertificateHash`,
  consommant `HttpsServerSession`/`TlsServerSession` (déjà livrés,
  `PRD-TLS.md`/`PRD-HTTP.md`) exactement comme FTPS (`PRD-FTP-SFTP.md` P8)
  les a déjà réutilisés pour un protocole différent — zéro nouvelle
  primitive TLS.
- **App pools** : `New-WebAppPool`/`Get-IISAppPool`, un modèle
  processus/isolement (identité, mode de recyclage, nombre de workers)
  purement métadonnées — **aucune exécution réelle de code .NET** (ce
  simulateur n'exécute jamais de code applicatif réel nulle part, cf.
  Oracle PL/SQL simulé, une distinction déjà actée dans le reste du
  projet).
- **Modules** : un registre statique des modules IIS déjà pertinents pour
  le comportement existant (`StaticFileModule`, `DefaultDocumentModule`,
  `HttpErrorsModule`), exposé par `Get-WebGlobalModule`, sans invocation
  dynamique de modules tiers.

**15. DFS Namespaces + DFSR.** `New-DfsnRoot`/`New-DfsnFolder` créent un
espace de noms logique (`\\domaine\namespace\dossier`) redirigeant vers une
ou plusieurs cibles `smbShares` réelles déjà livrées (P3) ; `New-DfsReplicationGroup`
réplique le contenu d'un dossier entre deux serveurs en réutilisant le
modèle USN/vecteur high-watermark de l'objectif 5 (réplication AD),
appliqué ici à des métadonnées de fichiers (nom, taille, hash de contenu,
horodatage) plutôt qu'à des objets d'annuaire — **même patron de
réplication, deuxième cas d'usage**, pas une seconde conception
indépendante.

**16. MS-RDPBCGR — RDP (négociation de connexion, sans rendu graphique).**
Port TCP 3389 : trame de connexion `X.224 Connection Request`/`Connection
Confirm` (RFC 905-descendant, réutilisé tel quel par MS-RDPBCGR §2.2.1),
négociation de sécurité (`RDP Negotiation Request/Response`), puis une
authentification de type CredSSP/NLA simplifiée (réutilise
`TlsClientSession`/`TlsServerSession` pour le canal, comme SSH/HTTPS/FTPS
déjà livrés). Une session RDP établie est un état de session réel
(visible via `query session`/`qwinsta`, `Get-RDUserSession`, fermeture par
`logoff`/`rwinsta`) — **aucun pipeline bitmap/graphique** n'est modélisé
(ce simulateur n'a de toute façon aucune surface de rendu distante nulle
part ; § 2.2, § 7 risque 2). C'est délibérément l'équivalent, pour RDP, de
ce que WinRM/SSH apportent déjà : authentification et session réelles,
zéro rendu.

**17. MS-CMRP — Clustering/failover (WSFC) minimal.** `New-Cluster`
regroupe 2+ `WindowsServer` en un cluster nommé ; un mécanisme de pulsation
(heartbeat) UDP périodique entre nœuds détecte la perte d'un nœud ; un
compte de quorum simple (majorité simple des nœuds vivants, **pas** de
disque témoin/partage de fichiers témoin réel) décide si le cluster reste
opérationnel ; `Add-ClusterFileServerRole` bascule la propriété d'un
groupe de ressources (ex. le rôle Serveur de fichiers) d'un nœud tombé
vers un nœud survivant — `Get-ClusterGroup`/`Move-ClusterGroup` reflètent
cet état.

**18. WSUS minimal.** Un rôle `WSUS` maintient un catalogue de mises à
jour fictives (identifiant KB, titre, catégorie, gravité — **aucun contenu
binaire de correctif réel**) ; les clients (`wuauclt`/`Get-WindowsUpdate`)
signalent leurs mises à jour « installées » à ce point WSUS plutôt qu'à
Windows Update directement (`Set-WUSettings -WUServer`) ; un administrateur
approuve/refuse une mise à jour par groupe de déploiement
(`Approve-WsusUpdate`).

**19. RFC 1179 — Impression réseau (Print Services).** Un rôle
`Print-Services` expose une file d'attente partagée (`Add-Printer
-ShareName`) accessible depuis un autre poste via LPD (RFC 1179, port 515,
commandes `Print any waiting jobs`/`Receive a printer job`/`Send queue
state`) ; `Get-PrintJob`/`Remove-PrintJob` gèrent la file. Réutilise la
structure `PrintJob` déjà définie côté client (`WinPrint.ts`) — **aucun
rendu réel de document**, seul l'accusé de réception/la progression de
file sont modélisés (même convention que `WinPrint.ts` lui-même).

**20. Activation/licences.** Un état de licence simulé par machine
(`Unlicensed`/`Licensed`/`OutOfBoxGrace`/`Notification`), `slmgr.vbs
/dlv`/`/ato`, `Get-CimInstance SoftwareLicensingProduct` reflètent cet
état ; une clé KMS/MAK factice suffit à faire transiter l'état vers
`Licensed`. Purement déclaratif — aucune vérification cryptographique
réelle de clé de licence.

**21. NPS avancé (modèles de stratégie complexes, comptabilité SQL).**
`New-NpsConnectionRequestPolicy` accepte des conditions multiples
combinées (groupe AD, heure du jour, type NAS, adresse d'accès), évaluées
dans l'ordre de priorité déjà établi par le moteur NPS existant ;
`Set-NpsAccountingConfiguration -SqlLogging` redirige les enregistrements
de comptabilité RADIUS déjà produits (`WindowsNpsRole`, livré) vers une
table simulée façon `OracleDatabase`/`OracleInstance` déjà présente dans ce
projet, plutôt que vers un fichier plat. **NAP (Network Access
Protection) est explicitement exclu** (§ 2.2) : NAP a été retiré de
Windows Server à partir de la 2012 R2 — le modéliser fidèlement pour une
identité « Windows Server 2022 » (déjà établie par
`PRD-Windows-Server.md` P1) serait une régression de fidélité, pas un
gain, malgré la demande générale de ce PRD de tout scoper sauf le GUI.

### 2.2 Non-objectifs (explicitement hors périmètre)

- **Hyper-V et toute forme de virtualisation/hyperviseur** — exclu
  explicitement (retour en arrière volontaire sur la v2.0 initiale de ce
  document, à la demande de l'utilisateur). Ce simulateur n'a nulle part de
  notion d'hôte pouvant contenir un autre device (§ 1, point 8) ; construire
  cette imbrication serait un changement architectural fondamental sans
  rapport avec le reste de ce PRD. Même un plan de gestion réduit (objets
  métadonnées sans VM réellement exécutable) reste hors périmètre — pas de
  cmdlets `*-VM`/`*-VMSwitch`, pas de rôle `Hyper-V`. Une éventuelle
  virtualisation mériterait son propre PRD dédié, pas un sous-objectif ici.

- **Cryptographie Kerberos bit-exacte** (AES-256-CTS-HMAC-SHA1-96/RC4-HMAC
  réels, dérivation de clé PBKDF2 conforme) — simulée, comme le reste du
  projet (TLS/QUIC/HTTP).
- **KCC réel** (calcul automatique de topologie de réplication inter-sites,
  coût de liens, fenêtres horaires, compression réseau) — remplacé par un
  cycle de réplication déclenché manuellement ou à intervalle fixe entre
  DC désignés explicitement.
- **Réplication SYSVOL** (DFSR/FRS) — le partage SYSVOL reste local à
  chaque DC, non synchronisé entre eux.
- **USN rollback, tombstones/lingering objects, garbage collection réelle**
  des objets supprimés.
- **PKINIT** (authentification Kerberos par certificat, RFC 4556),
  **FAST/armoring** (RFC 6113).
- **S4U2Self** (délégation non contrainte complète) — seul S4U2Proxy
  minimal (objectif 4) est couvert.
- **ACL de sécurité par attribut au niveau schéma** (confidentialité
  différenciée par attribut) — le modèle propriétaire/groupe/autres déjà
  livré par `PRD-Windows-Server.md` suffit.
- **Trusts avancés** (SID filtering, authentification sélective,
  quarantaine de domaine, trusts de raccourci) — un trust simple suffit.
- **Interface graphique** (ADSI Edit, « Active Directory Sites and
  Services », « Active Directory Domains and Trusts » MMC) — surface
  CLI/PowerShell uniquement, conformément au reste du simulateur.
- **Compression/chiffrement du canal de réplication** au-delà de ce que
  TLS apporterait s'il était explicitement activé (pas un objectif câblé
  par défaut).
- **RDP — tout pipeline graphique/bitmap** (négociation de connexion et
  authentification réelles seulement, § 2.1.16) — ce simulateur n'a aucune
  surface de rendu distante nulle part.
- **IIS — exécution réelle de code .NET** dans un app pool (métadonnées
  d'isolement seulement, § 2.1.14) — ce simulateur n'exécute jamais de
  code applicatif réel, cf. Oracle PL/SQL simulé.
- **Clustering — disque témoin/partage de fichiers témoin réel**, calcul
  de quorum dynamique pondéré — un compte de majorité simple suffit
  (§ 2.1.17).
- **Impression réseau — rendu réel de document** (accusé de réception/
  progression de file seulement, § 2.1.19, même convention que
  `WinPrint.ts`).
- **WSUS — contenu binaire réel de correctif**, télémétrie de conformité
  avancée — un catalogue de métadonnées suffit (§ 2.1.18).
- **Activation/licences — vérification cryptographique réelle de clé**
  — un état déclaratif suffit (§ 2.1.20).
- **NAP** (Network Access Protection) — retiré de Windows Server depuis la
  2012 R2 ; le modéliser contredirait l'identité « Windows Server 2022 »
  déjà établie (§ 2.1.21).

---

## 3. Architecture cible

### 3.1 Principe directeur

**Additif strict, aucune réécriture des sous-systèmes déjà livrés.** Le
schéma extensible, la forêt multi-domaines et la réplication étendent
`DirectoryTree`/`DirectoryStore` par de nouveaux champs et de nouvelles
partitions (`cn=schema`, `cn=configuration`) représentées comme des entrées
d'annuaire ordinaires — pas une structure de données parallèle. Kerberos
est un module entièrement nouveau (`src/network/kerberos/`) qui suit le
même patron que `src/network/tls/`/`src/network/quic/` : machine à états de
session, primitives cryptographiques simulées, encodage ASN.1 réel. Le
bind SASL/GSSAPI et les opérations LDAP manquantes s'ajoutent comme de
nouveaux `CHOICE` dans l'enveloppe `LDAPMessage` déjà réelle, sans toucher
au bind simple ni aux opérations déjà câblées.

### 3.2 Diagramme de couches

```
┌───────────────────────────────────────────────────────────────────────┐
│ Façades : cmdlets AD (New-ADDSDomainController, New-ADTrust, ...) +   │
│  cmdlets des nouveaux rôles (Install-AdcsCertificationAuthority,      │
│  New-WebBinding -Protocol https, New-DfsnRoot, New-Cluster,           │
│  Approve-WsusUpdate, Add-Printer -ShareName, slmgr.vbs,               │
│  New-NpsConnectionRequestPolicy) + commandes cmd (klist, netdom trust)│
├───────────────────────────────────────────────────────────────────────┤
│ Kerberos (RFC 4120)     │ Réplication/Forêt/Schéma  │ LDAP (étendu)    │
│  kerberos/KdcSession.ts │  ad/replication/           │ modifyDN/       │
│  kerberos/ticket.ts     │  ad/schema/                │ extended/       │
│  kerberos/crossRealm.ts │  ad/forest/                │ abandon/controls│
├───────────────────────────────────────────────────────────────────────┤
│ AD CS (WCCE)      │ IIS avancé (TLS)   │ DFS         │ RDP (RDPBCGR)   │
│  adcs/CaRole.ts    │  https/ (existant) │  dfs/       │  rdp/RdpSession │
│  → src/network/pki/│  → src/network/pki/│  (réutilise │  .ts (TPKT/X.224│
│  (réutilisé)       │  (réutilisé)       │  smbShares) │  + CredSSP/TLS) │
├───────────────────────────────────────────────────────────────────────┤
│ Clustering (WSFC) │ WSUS      │ Impression (LPD) │ Licences │ NPS avancé│
│  cluster/          │  wsus/    │  print/           │  licensing/│ nps/ │
│  ClusterService.ts │  WsusRole │  PrintServerRole  │  (RoleMgr) │ (ext)│
├───────────────────────────────────────────────────────────────────────┤
│ DomainJoinClient/DomainLogonClient — basculent sur Kerberos (§5 P24)  │
│ WinDomainDiag.ts (klist réel) — basculé sur l'état réel (§5 P2)       │
├───────────────────────────────────────────────────────────────────────┤
│ Sous-systèmes déjà livrés, tous inchangés :                           │
│  DirectoryStore/DirectoryTree/AdTypes · LdapServer/LdapClient/Ber ·   │
│  RoleManager/featureCatalog · src/network/pki/ (PRD-TLS.md) ·         │
│  HttpsServerSession/TlsServerSession (PRD-HTTP.md) · smbShares (P3) · │
│  WindowsNpsRole/RadiusServerAgent (PRD-RADIUS.md)                     │
├───────────────────────────────────────────────────────────────────────┤
│ TcpStack/TcpSocket (LDAP 389, Kerberos 88, HTTPS 443, RDP 3389, LPD   │
│ 515) · UDP (heartbeat cluster) · EventBus                             │
└───────────────────────────────────────────────────────────────────────┘
```

**Hyper-V n'apparaît volontairement nulle part ci-dessus** — exclu
explicitement (§ 2.2), y compris comme simple plan de gestion.

### 3.3 Modules proposés (arborescence)

```
src/network/kerberos/                    # NOUVEAU — protocole Kerberos entier
├── types.ts                             # PrincipalName, Realm, KDC-REQ/-REP, Ticket, EncTicketPart
├── KdcSession.ts                        # état serveur : AS-REQ/AS-REP, TGS-REQ/TGS-REP
├── KerberosClient.ts                    # état client : obtention TGT puis ticket de service
├── ticket.ts                            # encodage ASN.1 DER réel, chiffrement simulé
├── delegation.ts                        # S4U2Proxy minimal (objectif 4)
├── crossRealm.ts                        # référé cross-realm (objectif 10, s'appuie sur forest/TrustRelationship.ts)
├── events.ts                            # kerberos.as.*, kerberos.tgs.*, kerberos.delegation.*
└── observables.ts

src/network/devices/windows/server/ad/
├── DirectoryStore.ts                    # existant, étendu additivement : invocationId, USN par attribut
├── ldap/                                # existant, étendu additivement :
│   ├── LdapMessage.ts                   #   + modifyDNRequest/extendedRequest/abandonRequest, champ controls
│   ├── LdapControls.ts                  #   NOUVEAU — pagination RFC 2696, autres contrôles
│   └── (Ber/LdapDN/LdapFilter/LdapServer/LdapClient/DirectoryTree — inchangés)
├── replication/                         # NOUVEAU
│   ├── ReplicationSession.ts            #   cycle pull, échange de vecteurs high-watermark
│   ├── HighWatermarkVector.ts           #   USN par DC connu
│   ├── events.ts, observables.ts        #   replication.cycle.*, replication.conflict.*
├── schema/                              # NOUVEAU
│   ├── SchemaPartition.ts               #   cn=schema — attributeSchema/classSchema comme entrées DirectoryTree
│   └── SchemaValidator.ts               #   décore DirectoryTree.addEntry/modifyEntry
├── forest/                              # NOUVEAU
│   ├── Forest.ts                        #   partition de configuration, arborescence de domaines
│   ├── TrustRelationship.ts             #   trust simple bidirectionnel/transitif
│   └── sites.ts                         #   sites/sous-réseaux (objectif 7, métadonnées seules)
└── AdTypes.ts                           # existant, étendu additivement : AttributeSchema, ObjectClassSchema, TrustInfo

src/network/devices/windows/server/adcs/         # NOUVEAU — AD CS (§2.1.13)
├── CaRole.ts                             # service CertSvc, instancie une CertificateAuthority (src/network/pki/, réutilisée)
├── CertificateTemplate.ts                # gabarits minimaux (nom, EKU, durée)
└── events.ts, observables.ts             # adcs.certificate.issued/revoked

src/network/devices/windows/server/iis/
└── IisHttpsBinding.ts                    # NOUVEAU — §2.1.14, consomme HttpsServerSession/TlsServerSession (déjà livrés)
└── (WindowsIisRole.ts existant, étendu additivement : app pools/modules en métadonnées)

src/network/devices/windows/server/dfs/          # NOUVEAU — §2.1.15
├── DfsNamespace.ts                       # espace de noms logique → cibles smbShares (existant, réutilisé)
├── DfsReplicationGroup.ts                # réutilise le modèle USN/high-watermark de ad/replication/
└── events.ts, observables.ts

src/network/devices/windows/server/rdp/          # NOUVEAU — §2.1.16
├── RdpSession.ts                         # TPKT/X.224 Connection Request/Confirm, négociation de sécurité
├── CredSsp.ts                            # NLA simplifié, réutilise TlsClientSession/TlsServerSession
└── events.ts, observables.ts             # rdp.session.established/closed

src/network/devices/windows/server/cluster/      # NOUVEAU — §2.1.17
├── ClusterService.ts                     # adhésion, heartbeat UDP, quorum par majorité simple
├── ClusterResourceGroup.ts               # bascule d'un rôle (ex. Serveur de fichiers) entre nœuds
└── events.ts, observables.ts             # cluster.node.*, cluster.group.moved

src/network/devices/windows/server/wsus/         # NOUVEAU — §2.1.18
├── WsusRole.ts                           # catalogue de mises à jour (métadonnées), approbation par groupe
└── events.ts, observables.ts

src/network/devices/windows/server/print/        # NOUVEAU — §2.1.19
├── PrintServerRole.ts                    # file partagée, réutilise la structure PrintJob de WinPrint.ts
├── LpdTransport.ts                       # RFC 1179 réel (port 515)
└── events.ts, observables.ts

src/network/devices/windows/licensing/           # NOUVEAU — §2.1.20
└── LicensingState.ts                     # état Unlicensed/Licensed/Grace/Notification, slmgr.vbs

src/network/devices/windows/server/nps/
└── (WindowsNpsRole.ts existant, étendu additivement : §2.1.21 modèles de stratégie multi-conditions, comptabilité SQL — pas de NAP)
```

Note de frontière : ce PRD ne touche ni au moteur `DirectoryTree` (ajout de
champs seulement, jamais de réécriture de `search`/`addEntry`/
`modifyEntry`), ni au codec BER générique (`Ber.ts`), ni au parseur de DN/
filtres (`LdapDN.ts`/`LdapFilter.ts`), ni à `src/network/pki/`, ni à
`HttpsServerSession`/`TlsServerSession`, ni à `smbShares`, ni à
`WindowsNpsRole`/`RadiusServerAgent` — seules l'enveloppe `LdapMessage.ts`
(nouveaux `CHOICE`) et la validation d'entrée (`SchemaValidator.ts`, en
décorateur) sont étendues ; tout le reste est consommé strictement tel
quel. **Aucun répertoire `src/network/hyperv/` ou équivalent n'existe dans
cette arborescence** — Hyper-V est hors périmètre (§ 2.2).

### 3.4 Design patterns retenus

- **Machine à états explicite** (`KdcSession`, `KerberosClient`), à l'image
  de `TlsClientSession`/`TlsServerSession`.
- **Décorateur** (`SchemaValidator` enveloppant `DirectoryTree.addEntry`/
  `modifyEntry`) — même principe que `PermissionCheckingFSDecorator` côté
  SFTP : ajouter une vérification sans dupliquer le moteur qu'elle
  contrôle.
- **Strategy** pour le type de trust (bidirectionnel/entrant/sortant) et
  pour l'authentification LDAP (`simple` vs `GSSAPI`, deux implémentations
  de la même interface `IBindMechanism`).
- **Réutilisation stricte de la convention crypto simulée** déjà établie
  par `PkiKeyPair`/`SimulatedTls.ts`/le key schedule TLS — aucune nouvelle
  primitive cryptographique réelle introduite pour Kerberos.
- **Le schéma et la configuration comme données, pas comme code** :
  `attributeSchema`/`classSchema` sont des entrées `DirectoryTree`
  ordinaires (même mécanisme que n'importe quel autre objet AD), pas un
  fichier de constantes séparé — c'est ce qui permet à la réplication
  (objectif 5) de propager le schéma sans mécanisme dédié.
- **Réutilisation stricte de `src/network/pki/`** pour AD CS (§2.1.13) et
  de `HttpsServerSession`/`TlsServerSession` pour IIS HTTPS (§2.1.14) et
  RDP/CredSSP (§2.1.16) — aucune nouvelle primitive cryptographique dans
  tout ce PRD, Kerberos y compris (déjà noté ci-dessus).
- **Le même patron USN/vecteur high-watermark pour deux domaines
  différents** : réplication AD (objectif 5) et DFSR (§2.1.15) partagent
  la même conception, appliquée à deux types de données différents —
  pas deux implémentations indépendantes de « réplication ».
- **Adapter** (`PrintServerRole`/`LpdTransport`) au-dessus de la structure
  `PrintJob` déjà définie côté client (`WinPrint.ts`), à l'image de
  `SftpWireCodec`/`ScpWireCodec` (`PRD-FTP-SFTP.md`) adaptant une
  sémantique déjà existante vers un vrai protocole fil.

---

## 4. Modèle de données

### 4.1 KDC-REQ / KDC-REP / Ticket (RFC 4120 §5.4)

```
interface KdcRequest {
  readonly kind: 'AS-REQ' | 'TGS-REQ';
  readonly cname?: PrincipalName;         // absent en TGS-REQ (déjà dans le ticket joint)
  readonly realm: string;
  readonly sname: PrincipalName;
  readonly nonce: number;
  readonly till: number;                  // date d'expiration souhaitée
  readonly paData?: readonly PaData[];    // PA-ENC-TIMESTAMP, PA-TGS-REQ (ticket joint)
}

interface KdcReply {
  readonly kind: 'AS-REP' | 'TGS-REP';
  readonly cname: PrincipalName;
  readonly ticket: Ticket;                // chiffré pour le service cible
  readonly encPart: EncKdcRepPart;        // chiffré pour le client (clé simulée)
}

interface Ticket {
  readonly realm: string;
  readonly sname: PrincipalName;
  readonly encPart: EncTicketPart;        // "chiffré" — simulé, forme réelle
}

interface EncTicketPart {
  readonly flags: TicketFlags;            // forwardable/renewable/initial/pre_authent
  readonly key: SessionKey;
  readonly crealm: string;
  readonly cname: PrincipalName;
  readonly authtime: number;
  readonly starttime: number;
  readonly endtime: number;
  readonly renewTill?: number;
}
```

### 4.2 Réplication — USN et vecteur high-watermark (inspiré MS-DRSR)

```
interface DirectoryStoreReplicationState {
  readonly invocationId: string;          // identité stable du DC pour la réplication
  localUsn: number;                       // incrémentée à chaque écriture locale
}

interface AttributeStamp {
  readonly originatingInvocationId: string;
  readonly originatingUsn: number;
  readonly timestamp: number;
}

interface HighWatermarkVector {
  // par DC connu (invocationId) : la plus haute USN déjà reçue de ce DC
  readonly usnByInvocationId: ReadonlyMap<string, number>;
}
```

### 4.3 Schéma extensible (RFC 4512)

```
interface AttributeSchema {
  readonly ldapDisplayName: string;
  readonly attributeSyntax: string;       // OID de syntaxe (chaîne, entier, DN, booléen, ...)
  readonly isSingleValued: boolean;
}

interface ObjectClassSchema {
  readonly ldapDisplayName: string;
  readonly objectClassCategory: 'structural' | 'auxiliary' | 'abstract';
  readonly mustContain: readonly string[];
  readonly mayContain: readonly string[];
  readonly subClassOf?: string;
}
```

### 4.4 Forêt et trust

```
interface ForestDomain {
  readonly dnsName: string;
  readonly parentDnsName?: string;        // undefined => racine de forêt/arborescence
}

interface TrustRelationship {
  readonly localRealm: string;
  readonly remoteRealm: string;
  readonly direction: 'inbound' | 'outbound' | 'bidirectional';
  readonly transitive: boolean;
}
```

### 4.5 Contrôles et opérations LDAP additionnelles (RFC 4511)

```
interface LdapControl {
  readonly controlType: string;           // OID, ex. 1.2.840.113556.1.4.319 (paged results)
  readonly criticality: boolean;
  readonly controlValue?: Uint8Array;
}

interface ModifyDNRequest {
  readonly entry: string;                 // DN à renommer/déplacer
  readonly newRdn: string;
  readonly deleteOldRdn: boolean;
  readonly newSuperior?: string;
}
```

### 4.6 AD CS, IIS avancé, DFS

```
interface CertificateRequest {
  readonly template: string;               // nom du gabarit (§2.1.13)
  readonly subject: string;
  readonly publicKey: PkiPublicKey;        // réutilise src/network/pki/PkiKeyPair
}

interface IisHttpsBinding {
  readonly siteName: string;
  readonly port: number;                   // 443 par défaut
  readonly certificateThumbprint: string;  // référence un certificat déjà émis (AD CS ou auto-signé)
}

interface DfsNamespace {
  readonly namespacePath: string;          // \\domaine\namespace
  readonly folders: ReadonlyMap<string, readonly DfsFolderTarget[]>;
}
interface DfsFolderTarget {
  readonly serverName: string;
  readonly sharePath: string;              // cible smbShares existante
}
```

### 4.7 RDP, clustering, WSUS, impression, licences

```
interface RdpSessionState {
  readonly sessionId: number;
  readonly userName: string;
  readonly state: 'Active' | 'Disconnected';
  readonly clientAddress: string;
}

interface ClusterNode {
  readonly name: string;
  readonly state: 'Up' | 'Down';
  readonly lastHeartbeatAt: number;
}
interface ClusterResourceGroup {
  readonly name: string;
  readonly ownerNode: string;              // nœud qui possède actuellement le groupe
  readonly resourceType: 'FileServer';      // extensible ; seul le rôle Serveur de fichiers est couvert
}

interface WsusUpdate {
  readonly kbId: string;
  readonly title: string;
  readonly category: string;
  readonly severity: 'Critical' | 'Important' | 'Moderate' | 'Low';
}
interface WsusApproval {
  readonly kbId: string;
  readonly targetGroup: string;
  readonly action: 'Install' | 'Decline';
}

interface PrintJob {
  readonly id: number;
  readonly document: string;
  readonly owner: string;
  readonly submittedAt: number;
  readonly size: number;
  readonly status: 'Spooling' | 'Printing' | 'Completed' | 'Error';
}

type LicenseState = 'Unlicensed' | 'Licensed' | 'OutOfBoxGrace' | 'Notification';
```

---

## 5. Plan de mise en œuvre (TDD, par phases)

| Phase | Contenu | Dépend de |
|---|---|---|
| **P1 — KDC minimal, échange AS (RFC 4120 §3.1/§5.4)** | `kerberos/types.ts`/`KdcSession.ts`/`ticket.ts` : listener 88, `AS-REQ`/`AS-REP`, pré-authentification `PA-ENC-TIMESTAMP`, principal `krbtgt` auto-créé à la promotion du DC | — |
| **P2 — Échange TGS + `klist` réel (RFC 4120 §3.3)** | `TGS-REQ`/`TGS-REP`, tickets de service par SPN ; `WinDomainDiag.ts`/`cmdKlist` basculé sur l'état réel du cache de tickets | P1 |
| **P3 — Bind LDAP SASL/GSSAPI (RFC 4513 §5.2)** | Deuxième `AuthenticationChoice` dans `LdapMessage.ts`, consommant un ticket de service `ldap/<dc>` émis par P2 | P2, LDAP existant |
| **P4 — Réplication multi-DC : USN/vecteur high-watermark** | `ad/replication/ReplicationSession.ts`/`HighWatermarkVector.ts`, extension additive de `DirectoryStore`/`DirectoryTree` (invocationId, stamps par attribut) | — (indépendant de Kerberos) |
| **P5 — `Install-ADDSDomainController`** | Promotion d'un second DC dans un domaine existant, synchronisation initiale complète, `Get-ADDomainController` | P4 |
| **P6 — Sites (métadonnées)** | `ad/forest/sites.ts`, `New-ADReplicationSite`/`Get-ADReplicationSite`, annotation intra-/inter-site des journaux de réplication | P5 |
| **P7 — Schéma extensible (RFC 4512)** | `ad/schema/SchemaPartition.ts`/`SchemaValidator.ts`, partition `cn=schema`, cmdlets `New-ADAttribute`/`New-ADObjectClass` | — (indépendant de Kerberos/réplication) |
| **P8 — Forêt multi-domaines** | `ad/forest/Forest.ts`, partition de configuration partagée (y compris le schéma de P7), `New-ADDomain -ParentDomainName` | P7, P5 (la partition de configuration se réplique comme le reste) |
| **P9 — Trusts inter-domaine/inter-forêt + référé cross-realm** | `kerberos/crossRealm.ts`, `ad/forest/TrustRelationship.ts`, `New-ADTrust`/`netdom trust` | P8, P2 |
| **P10 — Délégation Kerberos contrainte (S4U2Proxy)** | `kerberos/delegation.ts`, attribut `msDS-AllowedToDelegateTo` | P2 |
| **P11 — LDAP filaire complet restant** | `modifyDNRequest`, `extendedRequest` (StartTLS), `abandonRequest`, champ `controls` + pagination RFC 2696, référés (`searchResultReference`, utile pour P8/P9) | LDAP existant, P8 (pour les référés) |
| **P12 — Observabilité AD DS/Kerberos** | `kerberos/events.ts`/`observables.ts`, `ad/replication/events.ts`/`observables.ts` transverses à P1-P11 | P1–P11 |
| **P13 — AD CS (Certificate Services)** | `adcs/CaRole.ts`/`CertificateTemplate.ts`, service `CertSvc`, `Install-AdcsCertificationAuthority`/`certreq -submit`/`Get-Certificate` — instancie une `CertificateAuthority` (`src/network/pki/`, réutilisée) | — (indépendant de Kerberos/réplication) |
| **P14 — IIS avancé : HTTPS/TLS** | `iis/IisHttpsBinding.ts`, `New-WebBinding -Protocol https`, consomme `HttpsServerSession`/`TlsServerSession` et, optionnellement, un certificat émis par P13 | P13 (pour un certificat AD CS) ou un certificat auto-signé existant |
| **P15 — IIS avancé : app pools et modules** | Modèle métadonnées `New-WebAppPool`/`Get-IISAppPool`, registre statique `Get-WebGlobalModule` | P14 |
| **P16 — DFS Namespaces + DFSR** | `dfs/DfsNamespace.ts` (cible `smbShares` existant), `dfs/DfsReplicationGroup.ts` (réutilise le modèle USN/high-watermark de P4) | P4 (réplication AD, patron réutilisé) |
| **P17 — RDP (négociation de connexion)** | `rdp/RdpSession.ts` (TPKT/X.224), `rdp/CredSsp.ts` (NLA simplifié sur `TlsClientSession`/`TlsServerSession`), `query session`/`Get-RDUserSession`/`logoff` | — (indépendant) |
| **P18 — Clustering/failover (WSFC) minimal** | `cluster/ClusterService.ts` (adhésion, heartbeat, quorum par majorité), `cluster/ClusterResourceGroup.ts` (bascule du rôle Serveur de fichiers) | — (indépendant) |
| **P19 — WSUS minimal** | `wsus/WsusRole.ts` (catalogue, approbation), clients redirigés (`Set-WUSettings -WUServer`) | — (indépendant) |
| **P20 — Impression réseau (Print Services)** | `print/PrintServerRole.ts`/`LpdTransport.ts` (RFC 1179 réel, port 515), réutilise `PrintJob` de `WinPrint.ts` | — (indépendant) |
| **P21 — Activation/licences** | `licensing/LicensingState.ts`, `slmgr.vbs`, `Get-CimInstance SoftwareLicensingProduct` | — (indépendant) |
| **P22 — NPS avancé (sans NAP)** | Extension additive de `WindowsNpsRole` : `New-NpsConnectionRequestPolicy` multi-conditions, `Set-NpsAccountingConfiguration -SqlLogging` vers une table simulée | `WindowsNpsRole` existant (`PRD-RADIUS.md`, livré) |
| **P23 — Observabilité transverse des nouveaux rôles** | `events.ts`/`observables.ts` pour `adcs.*`/`rdp.*`/`cluster.*`/`dfs.*` — mêmes conventions que P12 | P13–P22 |
| **P24 — Migration des consommateurs existants** | `DomainJoinClient.ts`/`DomainLogonClient.ts` basculent réellement sur Kerberos (P2/P3) au lieu du bind LDAP direct en clair ; comportement observable (jonction/logon réussissent ou échouent dans les mêmes cas) inchangé | P3 |

Chaque phase suit le cycle rouge → vert → refactor. Pendant P1-P23, ce
travail reste strictement additif (comme `PRD-FTP-SFTP.md` P1-P18) :
aucune suite existante (`windows-server-addsforest`, `windows-server-
domain-join`, `windows-server-ldap`, `windows-server-iis`, tests de
`klist`/`dcdiag`) n'est censée changer avant P24. **P24 change
délibérément ce principe**, pour la seule suite d'authentification de
domaine : son comportement observable (jonction/logon réussissent ou
échouent dans les mêmes cas qu'aujourd'hui) doit rester identique, seul le
mécanisme interne (Kerberos au lieu du bind LDAP direct) change. Les
phases P13 à P22 sont, comme les phases FTP/TFTP/SFTP/SCP de
`PRD-FTP-SFTP.md`, largement indépendantes entre elles — aucun ordre
imposé au-delà de leurs propres dépendances internes indiquées ci-dessus.

---

## 6. Stratégie de test

1. **Unitaires KDC (AS)** : `AS-REQ` sans pré-authentification → erreur
   `KRB-ERROR` `KDC_ERR_PREAUTH_REQUIRED` ; avec `PA-ENC-TIMESTAMP` valide
   → `AS-REP` contenant un TGT valide ; mot de passe incorrect → échec de
   déchiffrement de l'horodatage, pas de TGT émis.
2. **Unitaires KDC (TGS)** : un TGT valide obtient un ticket de service
   pour un SPN existant ; un TGT expiré ou falsifié est rejeté ; le ticket
   de service porte les bons `flags`/`crealm`/`cname`.
3. **Unitaires `klist`** : reflète exactement les champs du ticket en
   cache (heures, drapeaux, type de clé) — round-trip avec ce que P1/P2
   ont réellement émis, plus aucune chaîne figée.
4. **Unitaires bind SASL/GSSAPI** : un client muni d'un ticket `ldap/<dc>`
   valide se lie sans transmettre de mot de passe ; un ticket expiré ou
   pour le mauvais service est rejeté ; le bind `simple` existant continue
   de fonctionner sans régression.
5. **Unitaires réplication** : deux DC divergent (écriture locale sur
   chacun), un cycle de réplication converge vers le même état des deux
   côtés ; les vecteurs high-watermark empêchent de retransmettre un
   objet déjà reçu ; un conflit (même attribut modifié des deux côtés)
   se résout de façon déterministe et documentée (dernier écrivain gagne,
   par exemple, à définir explicitement).
6. **Intégration `Install-ADDSDomainController`** : un second serveur
   rejoint un domaine existant, obtient une copie complète de l'annuaire
   par synchronisation initiale, puis reste synchronisé après une
   écriture sur le premier DC.
7. **Unitaires sites** : un DC assigné à un site distinct est annoté
   « inter-site » dans le journal de réplication ; aucune propriété de
   coût de lien réel n'est testée (hors périmètre).
8. **Unitaires schéma** : création d'un `attributeSchema`/`classSchema`
   personnalisé, puis création d'un objet utilisant cette classe ;
   tentative de création d'un objet violant `mustContain` → échec propre ;
   les classes déjà livrées (`user`, `group`, …) continuent de fonctionner
   sans changement observable.
9. **Unitaires forêt** : un domaine enfant partage la même partition
   schéma que son parent (une modification de schéma sur le domaine racine
   se propage) ; `Get-ADForest` reflète l'arborescence.
10. **Unitaires/intégration trust** : un utilisateur du domaine A accède à
    une ressource du domaine B via un référé cross-realm ; sans trust
    configuré, l'accès échoue proprement (pas de repli silencieux).
11. **Unitaires délégation** : un service listé dans
    `msDS-AllowedToDelegateTo` obtient bien un ticket S4U2Proxy pour le
    compte de l'utilisateur ; un service non listé est rejeté.
12. **Unitaires LDAP filaire complet** : `modifyDNRequest` déplace/renomme
    effectivement un objet dans `DirectoryTree` ; `extendedRequest`
    StartTLS établit un vrai handshake TLS sur la connexion LDAP déjà
    ouverte ; `abandonRequest` interrompt une recherche en cours ; la
    pagination retourne des pages cohérentes et complètes sur un jeu de
    résultats plus grand qu'une page ; un référé est retourné pour une
    recherche visant un DN hors du domaine local dans une forêt multi-
    domaines.
13. **Unitaires/intégration AD CS** : une requête de certificat (`certreq
    -submit`) contre le rôle CA produit un certificat réellement signé et
    vérifiable par `CertificateVerifier` (`src/network/pki/`, déjà testé) ;
    un gabarit avec un usage restreint (EKU) rejette une requête hors
    périmètre.
14. **Unitaires/intégration IIS HTTPS** : `New-WebBinding -Protocol https`
    aboutit à un vrai handshake TLS 1.3 sur le port configuré ; un
    certificat expiré/non approuvé échoue proprement ; le contenu servi
    est identique en HTTP et en HTTPS pour un même site.
15. **Unitaires DFS** : un chemin logique de namespace redirige vers la
    bonne cible `smbShares` ; une écriture sur une cible se propage à
    l'autre après un cycle de réplication DFSR, en réutilisant exactement
    le même mécanisme de convergence que le test 5 (réplication AD).
16. **Unitaires/intégration RDP** : une négociation TPKT/X.224 aboutit à
    une session RDP établie et listée par `query session` ; des
    identifiants invalides échouent au niveau CredSSP/NLA, avant toute
    session ; `logoff` ferme la session proprement. Aucune assertion sur
    un contenu graphique (hors périmètre, § 2.2).
17. **Unitaires/intégration clustering** : deux nœuds forment un cluster ;
    l'arrêt simulé d'un nœud (heartbeat manquant) fait basculer le groupe
    de ressources Serveur de fichiers vers le nœud survivant ; un cluster
    à un seul nœud restant perd le quorum et le signale.
18. **Unitaires WSUS** : une mise à jour approuvée pour un groupe apparaît
    dans `Get-WindowsUpdate` d'un client membre de ce groupe ; une mise à
    jour refusée n'apparaît jamais.
19. **Unitaires/intégration impression réseau** : un job soumis depuis un
    autre poste via LPD apparaît dans la file partagée avec le bon
    statut ; `Remove-PrintJob` l'annule proprement.
20. **Unitaires licences** : l'application d'une clé KMS/MAK valide fait
    transiter l'état vers `Licensed` ; une clé invalide laisse l'état
    inchangé.
21. **Unitaires NPS avancé** : une stratégie de demande de connexion
    combinant plusieurs conditions (groupe + horaire) est évaluée dans le
    bon ordre de priorité ; les enregistrements de comptabilité
    apparaissent dans la table simulée quand `SqlLogging` est activé ;
    aucun test n'exerce de fonctionnalité NAP (absente par conception).
22. **Non-régression (P1-P23)** : exécution complète des suites AD DS/
    LDAP/domaine/IIS/NPS existantes après chaque phase, garantissant
    l'absence d'effet de bord tant que P24 n'est pas atteinte.
23. **Migration (P24)** : les suites de jonction/logon de domaine
    existantes ré-exécutées après bascule sur Kerberos — comportement
    observable (succès/échec dans les mêmes cas, mêmes messages
    d'erreur PowerShell/cmd) identique à l'avant-migration.

---

## 7. Risques et points d'attention

1. **Ampleur du chantier, considérablement élargie en v2.1** : ce PRD
   combine désormais douze familles de travail distinctes (Kerberos,
   réplication, schéma/forêt/trust/LDAP filaire, AD CS, IIS avancé, DFS/
   DFSR, RDP, clustering, WSUS, impression réseau, licences, NPS avancé)
   — significativement plus large que `PRD-FTP-SFTP.md` ou même
   `PRD-HTTP.md`. La contrepartie de cette ampleur : la plupart des
   nouvelles familles (AD CS, IIS HTTPS, DFS, RDP, clustering, WSUS,
   impression, licences, NPS avancé) sont **mutuellement indépendantes**
   au niveau du code (§ 5) — traiter chaque famille comme un sous-projet
   isolé, refuser tout ajout non listé en § 2.1 sans mise à jour explicite
   de ce document, et ne pas laisser une famille déborder sur une autre.
2. **Kerberos et la réplication sont entièrement greenfield** (§ 1.3) —
   contrairement à ce que le texte des non-objectifs de
   `PRD-Windows-Server.md` pouvait laisser penser, il n'y a **aucune base
   simplifiée à faire évoluer** : le risque n'est pas la régression d'un
   mécanisme existant mais la sous-estimation de la surface RFC 4120
   (types d'erreurs `KRB-ERROR`, interactions pré-authentification/
   renouvellement/délégation).
3. **Le schéma comme donnée d'annuaire est élégant mais fragile** : faire
   de `attributeSchema`/`classSchema` des entrées `DirectoryTree`
   ordinaires (§ 3.4) simplifie la réplication mais impose que
   `SchemaValidator` s'exécute **avant** toute écriture sur n'importe
   quelle partition — un chemin d'écriture qui contournerait le
   décorateur casserait silencieusement l'intégrité du schéma.
4. **Résolution de conflit de réplication non spécifiée par défaut dans le
   monde réel de manière simple** : MS-DRSR utilise un tampon de version
   par attribut assez subtil ; ce PRD doit choisir et documenter
   explicitement une règle déterministe (§ 6, test 5) plutôt que de la
   laisser implicite.
5. **Cross-realm et forêts multiplient les combinaisons de test** : une
   fois trust + forêt + délégation combinés, le nombre de scénarios
   d'authentification croît vite — prioriser les chemins P2/P3/P9/P10
   décrits en § 6 plutôt que d'essayer une couverture combinatoire
   complète.
6. **Ne pas dupliquer `DirectoryTree`** : la tentation de créer une
   structure de données séparée pour le schéma ou la configuration de
   forêt (plutôt que des entrées dans l'arbre existant) serait une
   régression architecturale et casserait la réplication uniforme de
   l'objectif 5.
7. **P24 est le seul point de bascule à risque de régression réelle** :
   toutes les phases précédentes sont additives ; ne pas anticiper la
   migration de `DomainJoinClient`/`DomainLogonClient` avant que P2/P3
   soient vertes et stables.
8. **Absence de journal de coordination dédié** : comme `PRD-FTP-SFTP.md`,
   ce document ne s'inscrit pas dans `docs/tls_quic_http_log.md` — un
   journal séparé devra être créé avant tout travail multi-agent sur ce
   PRD.
9. **Pas de dépendance bloquante externe, mais un ordonnancement interne
   strict** : contrairement à `PRD-QUIC.md`/`PRD-HTTP.md` à leur rédaction,
   aucune phase de ce document n'attend un chantier externe — mais
   tenter P3 avant P2, ou P9 avant P8, romprait les dépendances internes
   du tableau § 5.
10. **Hyper-V a été volontairement exclu après un premier arbitrage
    inverse** (« Historique des révisions », v2.0 → v2.1) : si une future
    demande souhaite le réintroduire, elle devra être traitée comme un
    nouveau PRD dédié plutôt qu'un ajout à celui-ci — l'architecture de ce
    document (façades additives sur des moteurs déjà livrés) ne prépare
    délibérément aucune imbrication hôte/invité, contrairement à ce qui
    avait été esquissé en v2.0.
11. **RDP sans pipeline graphique est une frontière délibérée, pas un
    manque à corriger plus tard** (§ 2.1.16, § 2.2) : ne pas interpréter
    l'absence de rendu comme une dette technique — ce simulateur n'a
    aucune surface de rendu distante nulle part, la frontière est
    cohérente avec le reste du projet (WinRM/SSH établissent aussi des
    sessions réelles sans rendu).
12. **Le clustering minimal (§ 2.1.17) simplifie fortement le quorum réel**
    (pas de disque témoin, pas de pondération de vote) — documenter
    clairement cette simplification dans le code et les tests plutôt que
    de laisser croire à une fidélité MS-CMRP complète.
13. **NAP est explicitement exclu de NPS avancé** (§ 2.1.21, § 2.2) malgré
    l'instruction générale de tout scoper sauf le GUI — c'est un choix
    documenté (fonctionnalité retirée de Windows Server 2012 R2+,
    incompatible avec l'identité « Windows Server 2022 » déjà établie),
    pas un oubli ; ne pas le réintroduire sans re-valider ce raisonnement
    avec l'utilisateur.
14. **WSUS/impression réseau/licences sont volontairement « fins »**
    (métadonnées et accusés de réception, § 2.1.18-20) — ne pas dériver
    vers une simulation de contenu binaire réel (correctifs, documents
    imprimés) qui dépasserait la fidélité visée par ce PRD sans valeur
    ajoutée pédagogique proportionnée.

---

## 8. Critères d'acceptation

1. Un utilisateur de domaine s'authentifie via `AS-REQ`/`AS-REP` puis
   `TGS-REQ`/`TGS-REP` réels ; `klist` affiche des tickets dont les champs
   sont dérivés de ce qui a effectivement été émis, plus aucune chaîne
   figée.
2. La jonction de domaine et le logon interactif (`DomainJoinClient`/
   `DomainLogonClient`) passent par Kerberos (bind LDAP SASL/GSSAPI), et
   non plus par un bind LDAP en clair — sans changement du comportement
   observable en cas de succès/échec.
3. Un second `WindowsServer` est promu DC additionnel d'un domaine
   existant (`Install-ADDSDomainController`), reçoit l'intégralité de
   l'annuaire par synchronisation initiale, puis reste convergent avec le
   premier DC après un cycle de réplication suivant une écriture locale
   sur l'un ou l'autre.
4. Un administrateur définit un `attributeSchema`/`classSchema`
   personnalisé et crée un objet de cette classe, qui survit à un cycle
   de réplication vers un second DC.
5. Une forêt à deux domaines (parent/enfant) partage effectivement le
   même schéma ; une modification de schéma sur le domaine racine se
   propage à l'enfant.
6. Un trust simple entre deux domaines permet à un utilisateur de l'un
   d'accéder, via un référé Kerberos cross-realm, à une ressource de
   l'autre ; sans trust, l'accès échoue proprement.
7. Un service autorisé par `msDS-AllowedToDelegateTo` obtient un ticket
   délégué (S4U2Proxy) pour le compte d'un utilisateur déjà authentifié.
8. `modifyDNRequest`, `extendedRequest` (StartTLS), `abandonRequest`, la
   pagination des résultats et les référés fonctionnent contre le serveur
   LDAP existant, sans régression des opérations déjà livrées (bind
   simple, search, add, modify, delete, compare).
9. La régression complète des suites AD DS/LDAP/domaine déjà livrées par
   `PRD-Windows-Server.md` reste verte à chaque phase P1-P23, et de
   nouveau verte après la migration P24 (comportement observable
   identique).
10. Un administrateur émet un certificat via AD CS (`certreq -submit`)
    et le lie à un site IIS (`New-WebBinding -Protocol https`) ; un
    client atteint ce site en HTTPS avec un handshake TLS 1.3 réel.
11. Un espace de noms DFS redirige vers deux cibles `smbShares` sur deux
    serveurs distincts, qui restent convergentes après un cycle DFSR
    suivant une écriture sur l'une des deux cibles.
12. Un client RDP négocie une connexion réelle (TPKT/X.224 + CredSSP/NLA)
    contre un `WindowsServer`, obtient une session listée par `query
    session`, puis se déconnecte proprement — sans qu'aucune assertion ne
    porte sur un contenu graphique.
13. Deux nœuds `WindowsServer` forment un cluster ; la perte simulée d'un
    nœud fait basculer le rôle Serveur de fichiers vers le survivant, visible
    via `Get-ClusterGroup`.
14. Une mise à jour approuvée par WSUS pour un groupe de déploiement
    apparaît dans `Get-WindowsUpdate` d'un client de ce groupe ; une mise à
    jour refusée n'apparaît jamais.
15. Un job d'impression soumis via LPD depuis un poste distant apparaît
    dans la file partagée d'un rôle Print Services, avec le bon statut de
    progression.
16. Une stratégie NPS combinant plusieurs conditions (groupe + horaire)
    est évaluée correctement, et les enregistrements de comptabilité RADIUS
    apparaissent dans la table SQL simulée quand la comptabilité SQL est
    activée — sans qu'aucune fonctionnalité NAP ne soit exercée ou
    revendiquée.
17. Hyper-V n'apparaît dans aucune surface de ce PRD (aucun cmdlet
    `*-VM`/`*-VMSwitch`, aucun rôle `Hyper-V`) — confirmé par une recherche
    de non-régression sur l'absence de ces artefacts après implémentation
    complète.

## §5 P4-bis — La résolution de conflit de réplication, et l'instabilité qu'elle causait

`scenario-ad-replication-topology.test.ts` échouait environ **une fois
sur vingt** en suite complète, sur un cas — « les rôles FSMO restent sur
DC01 après la promotion de DC02 » — qui passait systématiquement quand on
le lançait seul. Trois pistes ont été écartées par la mesure avant la
bonne : l'ordre des tests (mélangé cinq fois, toujours vert), un
singleton mal réinitialisé (`domainToForest` s'écrase à chaque
`createForest`), et le dépassement de délai (le cas dure 65 ms pour une
limite à 5 s).

### Le coupable : l'horloge

`EntryReplMeta.timestamp` vaut `Math.floor(Date.now() / 1000)` — en
SECONDES. Ce n'est pas le défaut : l'« originating time » du vrai AD a
la même résolution. Le défaut est que `applyReplicatedEntry` ne comparait
**que cela** :

```ts
if (existing.replMeta && existing.replMeta.timestamp > stamp.timestamp) return;
```

Pendant `Install-ADDSDomainController`, le contrôleur promu crée une
racine locale vide, puis tire celle de son partenaire. Quand les deux
créations tombaient dans deux secondes différentes — le laboratoire se
construit en ~50 ms, donc environ une fois sur vingt — la racine locale
passait pour « plus récente », l'entrée entrante était refusée, et les
attributs FSMO n'arrivaient jamais sur DC02.

### La mesure qui l'a établi

Un balayage de délais artificiels entre la création de DC01 et celle de
DC02, cinq essais chacun, en marquant les passages de seconde :

```
délai=   0ms  ok(seconde franchie)  ok  ok  ok  ok
délai=  30ms  KO(seconde franchie)  ok  ok  ok  ok
délai=  50ms  ok  ok  ok  KO(seconde franchie)  ok
délai= 100ms  ok  ok  ok  ok  KO(seconde franchie)
délai= 300ms  ok  ok  KO(seconde franchie)  ok  ok
```

Corrélation parfaite sur trente essais : **tout échec franchit une
seconde, aucun succès n'en franchit**. Ce n'est pas le délai qui compte,
c'est le découpage.

### Le correctif : la vraie règle d'AD

Documentation Microsoft à l'appui, un conflit se résout par **version**,
puis **heure d'origine**, puis **DSA d'origine** — dans cet ordre, et la
version est le critère principal *précisément parce que* l'heure a une
résolution d'une seconde. Il manquait la version.

`EntryReplMeta.version` compte les écritures de l'entrée et
`localWins(local, entrant)` applique les trois critères. La racine de
DC01 a été écrite plusieurs fois (création, puis initialisation des
FSMO) ; celle de DC02 vient de naître. La version tranche donc, quel que
soit l'endroit où tombe la seconde.

Le troisième critère n'est pas décoratif : à égalité parfaite il faut
trancher **de la même façon des deux côtés**, sinon les deux contrôleurs
divergent au lieu de converger. Comparer les identifiants de DSA donne
un résultat arbitraire mais stable, ce qu'un test vérifie dans les deux
sens.

### Ce qui n'a PAS été fait, et pourquoi

Passer l'horodatage en millisecondes aurait fait disparaître le symptôme
en divisant la fenêtre par mille, sans corriger la règle — et aurait en
prime éloigné le simulateur du vrai AD, dont l'heure d'origine est bien
en secondes. Le défaut n'était pas la précision de l'horloge, c'était de
s'en remettre à elle seule.

`ad-resolution-de-conflit-replication.test.ts` (7 cas) tient le
mécanisme et non le symptôme : il impose les horodatages au lieu
d'espérer tomber sur le bon découpage. Discriminé au git-stash, 6 des 7
échouent avant le correctif.
