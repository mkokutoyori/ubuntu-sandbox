...
| 13 | **Dette technique transport côté serveur** : la réponse est émise vers un port UDP **deviné** (`49152 + (identifier & 0x3fff)`) au lieu du port source du datagramme reçu, et vers une MAC **broadcast** — fragile, et faux dès que le client change son mapping de ports | RFC 2865 §2 | Moyenne |
| 14 | **Résolution d'egress client naïve** : correspondance de sous-réseau direct sinon « premier port up », MAC destination broadcast — **la table de routage de l'hôte est ignorée**, un serveur RADIUS joignable via une route (OSPF/statique) hors lien direct ne fonctionne pas proprement ; pas de `ip radius source-interface` réellement câblé au routage | — | Moyenne |
| 15 | Pas de **cache anti-duplication** côté serveur : une retransmission client est retraitée intégralement (double événement, double log) au lieu d'être répondue depuis un cache identifier+authenticator | RFC 2865 §3 | Faible |
...
| **P2 — Intégrité** | `authenticators.ts` : Response Authenticator conforme + **vérification client** (réponse invalide ignorée → retransmission), Message-Authenticator HMAC-MD5, Request Authenticator aléatoire ; correction du **port de réponse serveur** (port source du datagramme) et de la MAC destination | P1 |
...
| **P6 — EAP 802.1X & autorisation** | `eap.ts` (EAP-MD5), relais EAP-Message dans Dot1xAgent, attributs RFC 3580 ; `authorization.ts` : priv-lvl, Filter-Id, **VLAN dynamique**, Session-Timeout, Reply-Message | P3 |
...
| **P8 — Hébergement générique** | Contrat `RadiusServerHost` consommable par LinuxServer/WindowsPC ; egress client via table de routage + `source-interface` ; cache anti-duplication serveur | P2 |
