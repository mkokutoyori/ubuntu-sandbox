...
- **[MINEUR] `maximum-paths` sous `router bgp` est un no-op silencieux** : `CiscoRoutingProtoCommands.ts:253-263` ne traite que les branches `proto === 'rip'`/`'eigrp'` et retombe sur `return ''` pour `bgp` — la commande est acceptée sans erreur mais n'active jamais de multipath BGP (cohérent avec `computeLocRib()` qui ne retient qu'un seul meilleur chemin par préfixe, `BGPEngine.ts:309-332`). Risque pédagogique : l'apprenant croit avoir activé l'ECMP BGP alors que rien ne change.
