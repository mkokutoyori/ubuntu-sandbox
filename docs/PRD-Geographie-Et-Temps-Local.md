...
| **T5** ✅ | Un index non implanté est **refusé en le disant** (`unimplementedValues`), un index hors plage garde l'autre message | I-T4, §6 | `set timezone 55` accepté → UTC en silence. **Reste ouvert** : 79 index non tabulés, les trois sources étant bloquées et la seule liste obtenue décalée d'un cran (`TODO.md`) |
...
| **T8** ✅ | `SyslogAgent` lit l'horloge de sa machine par un port étroit ; `rfc5424Timestamp` vit dans `core/time` et porte le décalage | I-T5, RFC 5424 §6.2.3 | `show clock` disait 15 juillet 14:00 CEST pendant que le fil portait `Sep 10 10:38:02` — **deux mois d'écart**. Et `rfc5424Line` de FortiOS rendait toujours `Z` |
| **T9** ✅ | `date=`/`time=` portent l'heure LOCALE, par le port `localClock` du lot T8 ; `eventtime` reste l'instant absolu | fidélité, §3 | **la ligne de ce PRD était fausse** : les champs n'étaient pas absents, ils rendaient UTC. `execute time` disait 14:00:00 pendant que le journal écrivait `time=12:00:00` |
| **T10** ✅ | Fuseau de BASE et fuseau de SESSION ; `ALTER SESSION SET TIME_ZONE` agit et refuse par `ORA-01882` ; `SYS*` (serveur) et `CURRENT_*` (session) deviennent deux faits | I-T2, §6, §2 | `DBTIMEZONE`/`SESSIONTIMEZONE` rendaient **`null`** — la forme permissive de l'inconnu ; `ALTER SESSION` répondait « Session altered. » sans effet ; et **`SYSTIMESTAMP` et `CURRENT_TIMESTAMP` étaient la même écriture** alors qu'Oracle en fait deux faits. **Reste ouvert** : le format d'affichage `TIMESTAMP WITH TIME ZONE` (`TODO.md`) |
