...
| **R10** | ~~**La copie mise à jour incrémentalement**~~ **FAIT** — `BACKUP INCREMENTAL LEVEL 1 FOR RECOVER OF COPY WITH TAG '…' DATABASE` et `RECOVER COPY OF DATABASE\|DATAFILE n WITH TAG '…'` sont reconnus et évalués : le premier tour pose une copie image de niveau 0 et l'annonce, les suivants produisent le niveau 1, et `RECOVER COPY` le leur applique — la copie roule en avant. Une copie image porte désormais l'image du datafile (elle était écrite VIDE, donc restaurable de rien) | RMAN | la seule stratégie de sauvegarde d'Oracle qui n'exige jamais une complète de plus, et elle était intypable de bout en bout |
| **R11** | ~~**Deux critères lus, rendus, jamais évalués**~~ **FAIT** — `REPORT NEED BACKUP REDUNDANCY n` compte enfin les sauvegardes PAR DATAFILE et applique le n ; `NOLOGGING` supprime vraiment le redo, `V$NONLOGGED_BLOCK` porte la trace, `REPORT UNRECOVERABLE` nomme le fichier et une sauvegarde le lave | RMAN + applicative | un rapport qui dit « votre règle est satisfaite » quand elle ne l'est pas est pire que pas de rapport |
| **R12** | ~~**Le remède que RMAN prescrit**~~ **FAIT** — `CATALOG ARCHIVELOG`, `CATALOG START WITH '<préfixe>' [NOPROMPT]` et `CATALOG RECOVERY AREA` existent ; et `CATALOG BACKUPPIECE`/`DATAFILECOPY` regardent enfin ce que le fichier EST au lieu de croire le mot-clé | RMAN | le simulateur prescrivait un traitement qu'il n'acceptait pas |
| **R13** | **Data Guard — le transport** *(1/3)* — `LOG_ARCHIVE_DEST_n = 'SERVICE=…'` est ÉVALUÉ : un switch expédie le journal archivé sur une session Oracle Net réelle (LNS → RFS, découpé parce que l'en-tête NS est sur 16 bits), la standby l'écrit et l'enregistre dans son fichier de contrôle. Restent l'APPLICATION (`RECOVER MANAGED STANDBY DATABASE`) et la BASCULE | réseau + applicative | la limite nommée de CLAUDE.md, et le premier de ses trois morceaux |
| **R14** | **Data Guard — l'application** *(2/3)* — `ALTER DATABASE RECOVER MANAGED STANDBY DATABASE [CANCEL]` démarre et arrête un MRP qui applique vraiment les journaux reçus : une ligne insérée sur le primaire est interrogeable sur la standby, `CANCEL` fige le retard et une relance le rattrape, et `V$DATAGUARD_STATS` MESURE ce retard. Reste la BASCULE | applicative | la commande était avalée par le repli générique d'ALTER DATABASE, qui répond « Database altered. » à tout ce qu'il ne reconnaît pas |
| **R15** | **Data Guard — la bascule** *(3/3)* — le rôle est porté par l'instance et non écrit en dur ; `SWITCHOVER_STATUS` existe ; une standby REFUSE toute écriture (`ORA-16000`) au lieu de diverger en silence ; `SWITCHOVER TO` demande le rôle à l'autre base SUR LE FIL et `FAILOVER TO` ne demande rien à un primaire présumé perdu | applicative + réseau | deux bases se disaient PRIMARY au même instant, et la standby acceptait INSERT, COMMIT et CREATE TABLE |
| **R16** | **La commande par laquelle une standby NAÎT** — `DUPLICATE TARGET DATABASE FOR STANDBY [FROM ACTIVE DATABASE] [DORECOVER] [NOFILENAMECHECK]` est déclarée et évaluée : les datafiles et le fichier de contrôle du primaire TRAVERSENT le réseau (appel Oracle Net `ShipDatafile`, découpé comme le `ShipRedo` de R13), le serveur d'en face les charge dans SON stockage et les écrit sur SON disque, puis `alter database mount standby database` lui donne le rôle. Trois refus réels : `RMAN-06171` sans session AUXILIARY, `RMAN-05500` si l'auxiliaire est déjà montée, `RMAN-05001` si les noms de fichiers sont ceux de la cible et que `NOFILENAMECHECK` manque. Trois verrous levés en chemin : une instance NOMOUNT était injoignable alors que son `listener.ora` déclarait le SID en `SID_LIST_LISTENER` (deux vues du même fait qui se contredisaient), `ALTER DATABASE OPEN READ ONLY` était analysé puis ignoré (OPEN_MODE répondait READ WRITE et les écritures passaient), et une base MONTÉE répondait aux requêtes utilisateur au lieu d'`ORA-01219`. Enfin `RESTORE … PREVIEW` liste les sauvegardes qu'il emploierait et les trois SCN de reprise, au lieu d'un COMPTE | RMAN + applicative + réseau | R13-R15 avaient rendu réels le transport, l'application et la bascule ; il manquait la commande qui crée la standby, et RMAN n'avait aucune porte vers ce Data Guard |
