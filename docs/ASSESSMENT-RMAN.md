...
| **R4** | ~~**ARCHIVELOG** : mode, écriture du redo, `V$ARCHIVED_LOG`, `LOG SWITCH`~~ **FAIT** — une ligne écrite après la sauvegarde revient par `RECOVER` ; limite nommée : le journal porte un **instantané** au switch, pas un flux de vecteurs de changement | applicative | ouvre le PITR, `BACKUP ARCHIVELOG`, `RECOVER UNTIL` |
