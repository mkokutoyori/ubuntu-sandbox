...
| **D43** | Les garde-fous ajoutés d'après BRD-Firewall §40.6 sont numérotés **`G-P*`** et non `G*` : le BRD générique et le BRD FortiGate donnent chacun un sens à G6/G7/G8, et reprendre les chiffres aurait laissé croire que le fichier porte les huit du générique | E53 |
| **D44** | **Un garde-fou porte son témoin.** Chacun des trois nouveaux a un cas qui le fait échouer sur une entrée fabriquée — un garde qu'on ne voit jamais échouer ne prouve pas qu'il regarde. Celui de G-P1 a été vérifié en retirant pour de bon `sdwan` du pipeline FortiOS : il l'attrape | E53 |
