...
| **D81** | **Quand deux sessions écrivent le même mécanisme, on garde le plus GÉNÉRAL et on retire l'autre**, sans compromis : ici l'échange requête/réponse adressé de l'autre session sert aussi `execute ha manage`, ce qu'un drapeau sur une annonce diffusée ne peut pas faire. La sonde écrite à l'aveugle survit, elle, et devient un témoin indépendant — 6 de ses 7 cas passent sur un mécanisme qu'elle n'a pas vu naître | E74 |
...
|---|---|---|
| 2026-08-22 | agent `mandeng` | Collision de mécanismes sur la traction HA (E74). Décision D81. **Mon implémentation est retirée au profit de celle de l'autre session**, plus générale ; ma sonde écrite à l'aveugle est gardée et passe sur leur mécanisme. |
