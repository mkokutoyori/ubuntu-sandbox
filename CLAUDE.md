...
- **Data Guard ships and applies redo; the role switch is still a label.** `LOG_ARCHIVE_DEST_n = 'SERVICE=…'` is evaluated: a log switch sends the archived log to the standby over a real Oracle Net session (LNS → RFS, segmented because the NS header is 16-bit), the standby writes it and records it in its own control file, and `ALTER DATABASE RECOVER MANAGED STANDBY DATABASE [CANCEL]` starts/stops an MRP that really applies it — a row inserted on the primary is queryable on the standby, and `V$DATAGUARD_STATS` measures the apply lag. What is NOT built: `switchover()` still swaps two role fields without moving data, and nothing enforces which side may be written.
