...
- **Data Guard ships redo but does not apply it yet.** `LOG_ARCHIVE_DEST_n = 'SERVICE=…'` is evaluated: a log switch sends the archived log to the standby over a real Oracle Net session (LNS → RFS, segmented because the NS header is 16-bit), the standby writes it and records it in its own control file, and `V$ARCHIVE_DEST`/`V$ARCHIVE_DEST_STATUS` report the outcome. What is NOT built: `ALTER DATABASE RECOVER MANAGED STANDBY DATABASE` still answers `Database altered.` without applying anything, so the standby's SCN never advances; and `switchover()` still swaps two role fields without moving data.
