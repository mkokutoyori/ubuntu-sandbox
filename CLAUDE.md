...
- `nfs/` — NFSv3 on the wire: `wire/` carries XDR (RFC 4506), ONC RPC with AUTH_SYS and stream record marking (RFC 5531), the 22 NFSv3 procedures, MOUNT v3 and portmap (RFC 1833). One `RpcService` hosts all three programs. `ExportTable` parses `/etc/exports` and its options DECIDE (`ro` → `NFS3ERR_ROFS`, `root_squash` squashes before the disk is touched). A client-side mount plugs into the VFS through `RemoteMountPort`, so `cat`, `cp`, `mv` and RMAN's piece writes all cross the wire unchanged. The RFC text is unreachable from this environment; the numbers and layout come from the Linux uapi headers and `fs/nfsd/nfs3xdr.c`, and the probes say so.
...
- **Oracle PDBs share one schema namespace** — `ALTER SESSION SET CONTAINER` swaps a label only.
- **Data Guard is real end to end — but there is no broker.** `LOG_ARCHIVE_DEST_n = 'SERVICE=…'` is evaluated: a log switch ships the archived log to the standby over a real Oracle Net session (LNS → RFS, segmented because the NS header is 16-bit); `ALTER DATABASE RECOVER MANAGED STANDBY DATABASE [CANCEL]` starts/stops an MRP that really applies it, so a row inserted on the primary is queryable on the standby; a `PHYSICAL STANDBY` refuses every write (`ORA-16000`); and `SWITCHOVER TO` / `FAILOVER TO` move the role — the switchover asking the other side **over the wire**, so a severed network leaves both roles unchanged rather than creating two primaries. What is NOT built: DGMGRL and the broker configuration, `MAXIMUM PROTECTION` enforcement (the protection mode is stored and displayed, never enforced), standby redo logs as files, and cascaded standbys.
